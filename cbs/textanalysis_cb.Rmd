...
This tutorial is the interactive Jupyter notebook accompanying the [Language Technology and Data Analysis Laboratory (LADAL) tutorial on text analysis and distant reading](https://slcladal.github.io/textanalysis.html). This tutorial is aimed at beginners and intermediate users of R with the aim of showcasing how to perform basic text analytic procedures on textual data using R. The aim is not to provide a fully-fledged analysis but rather to show and exemplify selected useful methods associated with text analysis and distant reading.

Text Analysis (cf. Bernard, and Ryan 1998, Kabanoff 1997, Popping 2000) is the computer-based analysis of language data or the (semi-)automated extraction of information from text. While Text Analysis encompasses a wide range of methods, this tutorial will only serve as a very basic introduction that highlights selected, commonly used techniques.   
...
![Google's N-gram viewer.](https://slcladal.github.io/images/GoogleNgram.png)
...
![Network of persona's in Shakespeare's Romeo and Juliette.](https://slcladal.github.io/images/romeonet.png)
...
This tutorial is based on R. If you are new to R, you will find an introduction to it and more information on how to use R [here](https://slcladal.github.io/intror.html). For this tutorials, we need to install certain *packages* from an R *library* so that the scripts shown below are executed without errors. Before turning to the code below, please install the packages by running the code below this paragraph - it may take some time (between 1 and 10 minutes to install all of the packages for the first time so you do not need to worry if it takes some time).
...
![KWIC concordances produced with AntConc](https://slcladal.github.io/images/AntConcConcordance.png)
...
The output shows that the data still consists of short text snippets but before we combine them, we will briefly see how to upload and use your own data.
...
# create wordcloud
wordcloud::wordcloud(words = wfreq_wostop$word, 
                     # frequencies
                     freq = wfreq_wostop$freq, 
                     # n of words
                     max.words=100,
                     # define colors
...
A very common procedure to add information to texts (also known as annotating texts) is to part-of-speech tag the data, which means to determine to what type of word a specific word belongs. Below, we will add pos-tags to a short English text. Here, we will use the *udpipe* R package. UDPipe was developed at the Charles University in Prague and the `udpipe` R package  (Wijffels 2021) is an extremely interesting and really fantastic package as it provides a very easy and handy way for language-agnostic tokenization, pos-tagging, lemmatization and dependency parsing of raw text in R. 

We begin by loading a short text that we will the pos-tag..
...
We could, for example, use pos-tagging to check differences in the distribution of word classes across different registers or to find certain syntactic patterns in a collection of texts. We could also download and use language models for other languages and pos-tag, e.g., a German, Japanese, French, Chinese, or Russian text. If you want to know more, please check out the the [Language Technology and Data Analysis Laboratory (LADAL) tutorial on part-of-speech annotation and syntactic parsing](https://slcladal.github.io/tagging.html).

# Citation & Session Info 

Schweinberger, Martin. 2022. *Text Analysis and Distant Reading using R*. Brisbane: The University of Queensland. url: https://slcladal.github.io/textanalysis.html.
...
[Back to LADAL](https://slcladal.github.io/index.html)
