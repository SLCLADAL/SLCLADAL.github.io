...
```{r}
# load function that extract keyness measures (default ordering by RateDifference)
...
Here we will use a simple bar plot to visualize the 10 top keywords of both texts. The bar plot uses the *phi* value but you are welcome to visualize another association measure (simply replace `phi` with the measure you want to use).
...
rbind(top, bot) %>%
  # create a ggplot
  ggplot(aes(x = reorder(token, phi, mean), y = phi, label = phi, fill = type)) +
  # add a bar plot using the 'phi' values
  geom_bar(stat = "identity") +
  # add text labels above the bars with rounded 'phi' values
  geom_text(aes(y = ifelse(phi > 0, phi - 0.05, phi + 0.05), 
                label = round(phi, 3)), color = "white", size = 3) + 
