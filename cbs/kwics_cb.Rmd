...
For this tutorial, the default data represents the text of Charles Darwin's *On the Origin of Species by means of Natural Selection* which we download from the [GitHub data repository of the *Language Technology and Data Analysis Laboratory* (LADAL)](https://slcladal.github.io/data). 
...
```{r basekwic4}
nrow(kwic_natural); length(kwic_natural$keyword)
```
...
kwic_natural_longer <- kwic(text, pattern = "selection", 
...
kwic_naturalselection <- kwic(text, pattern = phrase("natural selection"))
# inspect
...
kwic_pipe <- kwic(x = text, pattern = "selection") %>%
...
kwic_ordered <- kwic(x = text, pattern = "natural") %>%
  dplyr::arrange(post)
# inspect
kwic_ordered %>%
  as.data.frame() %>%
