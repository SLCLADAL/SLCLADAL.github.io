...
This tutorial introduces sentiment analysis (SA) and show how to perform a SA in R. The entire R-markdown document for the tutorial can be downloaded [here](https://slcladal.github.io/rscripts/sentiment.Rmd). SA is a cover term for approaches which extract information on emotion or opinion from natural language . SA have been successfully applied to analysis of language data in a wide range of disciplines such as psychology, economics, education, as well as political and social sciences. Commonly SA are used to determine the stance of a larger group of speakers towards a given phenomenon such as political candidates or parties, product lines or situations. Crucially, SA are employed in these domains because they have advantages compared to alternative methods investigating the verbal expression of emotion. One advantage of SA is that the emotion coding of SA is fully replicable.
...
```{r prep2, echo=F, eval = F, message=FALSE, warning=FALSE}
...
Sentiment Analysis (SA) extracts information on emotion or opinion from natural language . Most forms of SA provides information about positive or negative polarity, e.g. whether a tweet is *positive* or *negative*. As such, SA represents a type of classifier that assigns values to texts. As most SA only provide informarion about polarity, SA is often regarded as rather coarse-grained and, thus, rather irrelevant for the types of research questions in linguistics. 
...
```{r sa7b, echo = F, message=FALSE, warning=FALSE}
# inspect data
darwin_clean %>%
  head(10) %>%
...
```{r bsa1b, echo = F, message=FALSE, warning=FALSE}
# inspect data
novels_anno %>%
  head(10) %>%
...
```{r bsa3b, echo = F, message=FALSE, warning=FALSE}
# inspect data
novels %>%
  head(10) %>%
...
```{r contribsa1b, echo = F, message=FALSE, warning=FALSE}
# inspect data
novels_impw %>%
  head(10) %>%
...
```{r bin1b, echo = F, message=FALSE, warning=FALSE}
# inspect data
novels_bin %>%
  head(10) %>%
...
```{r ma1b, echo = F, message=FALSE, warning=FALSE}
# inspect data
novels_change %>%
  head(10) %>%
...
```{r neg1b, echo = F, message=FALSE, warning=FALSE}
# inspect data
orwell_sent %>%
  head(10) %>%
...
If you are interested in learning more about SA in R,is highly recommended as it goes more into detail and offers additional information.
