...
There are several different R packages that assist with pos-tagging texts . In this tutorial, we will use the `openNLP`, the `corNLP`, and the `TreeTagger` packages. Each of these has advantages and shortcomings and it is advantageous to try which result best matches one's needs.
...
# inspect  results
datatable(postagged rownames = FALSE, options = list(pageLength = 5, scrollX=T), filter = "none")
```
...
```{r, tree8}
postaggedtext <- paste(postagged postagged sep = "/", collapse = " ")
postaggedtext
```
...
# inspect  results
datatable(postagged_german rownames = FALSE, options = list(pageLength = 5, scrollX=T), filter = "none")
```
...
# inspect  results
datatable(postagged_french rownames = FALSE, options = list(pageLength = 5, scrollX=T), filter = "none")
```
...
# inspect results
datatable(postagged_spanish rownames = FALSE, options = list(pageLength = 10, scrollX=T), filter = "none")
```
...
# inspect results
datatable(postagged_italian rownames = FALSE, options = list(pageLength = 5, scrollX=T), filter = "none")
```
...
# inspect results
datatable(postagged_dutch rownames = FALSE, options = list(pageLength = 5, scrollX=T), filter = "none")
```
...
Another package that is very handy when it comes to pos-tagging but more importantly syntactic parsing is the `coreNLP` package . Unfortunately, we cannot use it at this point as it is not supported by up-to-date versions of R.
...
Schweinberger, Martin. 2020. *POS-Tagging and Syntactic Parsing with R*. Brisbane: The University of Queensland. url: https://slcladal.github.io/tagging.html (Version `r format(Sys.time(), '%Y.%m.%d')`).
