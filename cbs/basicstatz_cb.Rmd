...
If the reported skewness value is negative, then  the distribution is positively skewed. If the value is positive, then the distribution is negatively skewed. If the value is lower than -1 or greater than +1, then the distribution can be assumed to be substantively skewed (Hair et al. 2017).  
...
The Levene's test (cf. Levene 1962) evaluates if the variances of two groups is approximately equal - this is referred to as homoskedasticity. This is important, because unequal variances - or heteroskedasticity - strongly suggest that there is another factor, a confound, that is not included in the model but that significantly affects the dependent variable which renders results of an analysis unreliable.
...
Chi-square tests depend on certain requirements that, if violated, negatively affect the reliability of the results of the test. To provide reliable results, 80 percent of cells in a table to which the chi-square test is applied have to have expected values of 5 or higher and at most 20 percent of expected values can be smaller than 5 (see Bortz et al. 1990: 98). In addition, none of the expected values can be smaller than 1 (see Bortz et al. 1990: 136) because then, the estimation, which relies on the $\chi$^2^-distribution, becomes too imprecise to allow meaningful inferences (Cochran, William 1954).  
...
In this first example, we are dealing with a table consisting of two columns and multiple rows, a so-called 2\*k table (two-by-k table). In order to test if a feature combination, that is represented by a row in the 2\*k table, is significantly more common compared with other feature combinations, we need to implement the $\chi$^2^-equation from (Bortz et al. 1990: 126-127). 
...
The correct analysis takes into account that it is a sub-table that is not independent of the overall table. This means that the correct analysis should take into account the total number of cases, as well as the row and column totals (cf. Bortz et al., 144-148].

In order to perform the correct analysis, we must either implement the equation proposed in Bortz et al. (1990) or read in the function written by Gries (2014) and apply it to the sub-table.
...
Schweinberger, Martin. 2021. *Basic Inferential Statistics with R*. Brisbane: The University of Queensland. url: https://slcladal.github.io/basicstatz.html.
...
Bortz, Jürgen, Gustav Adolf Lienert, and Klaus Boehnke. 1990. *Verteilungsfreie Methoden in Der Biostatistik*. Berlin: Springer Verlag.

Bühner, Markus and Matthias Ziegler. 2009. *Statistik Für Psychologen Und Sozialwissenschaftler*. München: Pearson Studium.

Cochran, William G. 1954. Some Methods for Strengthening the Common $\chi$^2tests. *Biometrics* 10: 417–51.

Cohen, Jacob. 1988. *Statistical Power Analysis for the Behavioral Sciences*. Hillsdale, NJ: Erlbaum.

Gries, Stefan Thomas. 2014. "Frequency Tables: Tests, Effect Sizes, and Explorations." In Dylan Glynn and Justyna Robinson (eds.), *Polysemy and Synonymy: Corpus Methods and Applications in Cognitive Linguistics*, 365–89. Amsterdam: John Benjamins.

Hair, Joseph F., Hult, G. Tomas M., Ringle, Christian M., Sarstedt, Marko, and Thiele, Kai Oliver. 2017. “A Primer on Partial Least Squares Structural Equation Modeling (Pls-Sem).” *Journal of the academy of marketing science* 45(5): 616-632.

Levene, Howard. 1962. "Robust Tests for Equality of Variances." In Burkholder, D. L. (ed.), *Contributions to Probability and Statistics. Essays in Honor of Harold Hotelling*, 279–292.
