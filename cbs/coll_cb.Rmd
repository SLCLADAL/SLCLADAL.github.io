...
source(here::here("rscripts/colldf.R"))
...
source(here::here("rscripts/assocstats.R"))
...
```{r}
stats_tb <- freq_df %>%
  dplyr::mutate(`kwic n words` = sum(kwic),
                corpuswokwic = corpus-kwic,
                `corpus n words` = sum(corpuswokwic)) %>%
  dplyr::mutate(O11 = kwic,
                O12 = `kwic n words` - kwic,
                O21 = corpuswokwic,
                O22 = `corpus n words` - corpuswokwic) %>%
  dplyr::select(-corpus, -kwic, -`kwic n words`, -`corpus n words`,-corpuswokwic)
# inspect resulting frequency list
head(stats_tb, 10)
```
...
|co-occurrence with keyword   | Collocate (e.g., said) | other tokens |
 ---------------------------- | ---------------------- | ------------ |
| kwic                        | a                      | b            |
| corpus (wo kwic)            | c                      | d            |
