...
This tutorial focuses on tree-based models and their implementation in R. The entire R markdown document for this tutorial can be downloaded [here](https://slcladal.github.io/tree.Rmd). For the more advanced, a recommendable resource for tree-based modeling isorVery good papers dealing with many critical issues related to tree-based models areand 
...
# install libraries
install.packages(c("Boruta", "caret", "cowplot", "tidyverse", 
                   "ggparty",  "Gmisc", "grid", "Hmisc", 
...
options(stringsAsFactors = F)         # no automatic data transformation
options("scipen" = 100, "digits" = 4) # suppress math annotation
# load packages relevant for the Markdown document but not the stats and vis
library(flextable)
library(DT)
```

# Tree-Structure Model Basics
...
Despite these potential advantages, a word of warning is in order:admits that tree-based models can be very useful but there are some issues that but some serious short-comings of tree-structure models remain under-explored. For instance, 
...
2. Simple tree-structure models have been shown to fail in detecting the correct predictors if the variance is solely determined by a single interaction . This failure is caused by the fact that the predictor used in the first split of a tree is selected as the one with the strongest main effect . This issue can, however, be avoided by hard-coding the interactions as predictors plus using ensemble methods such as random forests rather than individual trees . 

3. Another shortcoming is that tree-structure models partition the data (rather than "fitting a line" through the data which can lead to more coarse-grained predictions compared to regression models when dealing with numeric dependent variables [again, seechapter 7.3]. 

4.341 state that high correlations between predictors can hinder the detection of interactions when using small data sets. However, regression do not fare better here as they are even more strongly affected by (multi-)collinearity (seechapter 7.3).
...
The most basic type of tree-structure model is a decision tree. An extension of decision trees are so-called classification and regression trees (CARTs) or conditional inference trees (CITs). The difference between simple decision trees and CITs is that CITs use significance tests, e.g. the p-values, to select variables rather than some information measures like the Gini coefficient .


Below is a decision tree which shows what what response to expect, in this case whether a speaker uses discourse *like* or not. Decision trees, like CARTs and CITs, answer a simple question, namely *How do we classify elements based on the given predictors?*. The answer that decision trees provide is the classification of the elements based on the levels of the predictors. In simple decision trees, all predictors, even those that are not significant are included in the decision tree. The decision tree shows that the best (or most important) predictor for the use of discourse *like* is age as it is the highest node. Among young speakers, those with high status use *like* more compared with speakers of lower social status. Among old speakers, women use discourse *like* more than men.


```{r cit1a, echo = F, message=FALSE, warning=FALSE}
# example of a simple decision tree
library(tree)
# set options: do not convert strings
...
In a first step, we initialize our R session by setting options and activating the necessary packages.
...
# inspect data
ft <- flextable(head(citdata,10)) %>%
  set_caption("First 10 rows of the citdata data set.")
ft
```

An interactive display of the data is provided below.
...
```{r cit4e, echo=T, message=FALSE, warning=FALSE}
...
```{r rf10, message=FALSE, warning=FALSE}
...
```{r rf11, message=FALSE, warning=FALSE}
...
The error rate is very similar across different values of `mtry` but the lowest values is reported if all 4 variables are considered at once. We can now create a pruned or optimized RF based on the previous recommendations (ntree = 30, mtry = 4). Again, we begin by setting a seed.
...
Schweinberger, Martin. 2021. *Tree-Based Models in R*. Brisbane: The University of Queensland. url: https://slcladal.github.io/tree.html (Version `r format(Sys.time(), '%Y.%m.%d')`).
