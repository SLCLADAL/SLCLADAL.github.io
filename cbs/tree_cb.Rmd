...
NOTE: In some cases, installing the `caret` package can be a bit more complicated. In my case, it was necessary to execute the code chunk shown below. However, once the `caret` package is installed, you do not need to go through these steps again and can simply activate it by calling `library(caret)`.
...
How to prune and evaluate the accuracy of decision trees is not shown here. If you are interested in this, please check out chapter 7 ofwhich is a highly recommendable resource that provide a lot of additional information about decision trees and CARTs. 
...
```{r cit4f, echo=T, message=FALSE, warning=FALSE}
