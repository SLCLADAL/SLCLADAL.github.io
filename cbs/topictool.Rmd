...
```{r}
cf <- list.files(here::here("data/tcorpus"), full.names = T)
corpus <- sapply(cf, function(x){
  x <- scan(x, what = "char")
  x <- paste0(x, collapse = " ")
})
str(corpus)
...
tmlda <- topicmodels::LDA(clean_dfm, k = 4, control = list(seed = 1234))
```
...
```{r}
# inspect
tidytext::tidy(tmlda, matrix = "beta") %>%
