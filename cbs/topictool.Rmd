...
<b>IMPORTANT: topic modeling works best if the text are neither too short nor too long. Typically  paragraphs work best (texts above sentence level and below article length).</b>
<br>
</p>
...
corpus_clean <- 
  ...
  stringr::str_squish()
toks_corpus <- tokens(corpus, 
                      remove_punct = TRUE, 
                      remove_numbers = TRUE, 
                      remove_symbol = TRUE) %>%
  tokens_remove(toks_corpus, 
                pattern = c(stopwords("en"), "these", "are", "example", "words"))
...
Here we look for 10 topics but we would vary the number of topics (k) to check what topics are in our data.

```{r}
# set seed
set.seed(1234)
# generate model: change k to different numbers, e.g. 5 or 15 and look for consistencies in the keywords for the topics below.
tmod_lda <- seededlda::textmodel_lda(dfmat_corpus, k = 10)
# inspect
terms(tmod_lda, 10)
```
