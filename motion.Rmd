...
In this example, we want to analyze if the percentage with which an adjective is modified by *very* changes over time. 
...
library(DT)
# load data
coocdata <- read.table("https://slcladal.github.io/data/collex_decade.txt", sep = "\t", header = T)
# inspect data
head(coocdata, 10)
```
...
After generating the motion chart, we not plot it.
