...
Wir wenden uns nun einem sehr weit verbreiteten statistischen Verfahren zu, der Regression. Wir werden zunächst den einfachen Fall einer einfachen Regression mittels zweier Beispiele anschauen, um relevante Konzepte besser zu verstehen. Anschließend werden wir uns der multivariaten oder multiplen linearen Regression zuwenden. Der \verb!R!-Code, der in den Beispielen verwendet wird, basiert auf \citet{field2012discovering}.
Regressionen werden genutzt, um zu schauen, ob eine oder mehrere der unabhängigen Variablen (oder Prädikatoren) mit einer abhängigen Variable korrelieren.
...
Die Idee einer Regression ist in Formel (\ref{eq:slm}) dargestellt und lässt sich am Besten graphisch darstellen: Stellen sie sich eine Grade vor, die durch eine die Punkte in einem Koordinatensystem gezogen wird (Grafik \ref{fig:scat1} linkes Panel).
...
Regressionen versuchen nun, diejenige Grade zu finden, die insgesamt den geringsten Abstand zu den Punkten hat (Grafik \ref{fig:scat1} mittleres Panel). Technisch gesprochen besteht das Ziel einer Regression also darin diejenige Grade zu finden, die die geringste Summe der Residuen (Varianz) hat (Grafik \ref{fig:scat1} rechtes Panel). Hiermit ist gemeint, dass die Summe der Längen der Linien, die man von der Graden weg zu den jeweiligen Punkten führen, so klein wie möglich sein soll. Die Neigung dieser Regressionsgeraden wird Koeffizient genannt und der Punkt an dem die Regressionsgerade die y-Achse schneidet wird als Intercept bezeichnet.

\begin{figure}[H]
\centering
\includegraphics[width=1\textwidth]{images/scat1.png}\\[.25cm]
\caption{Scatterplots mit Regressionsgeraden (mitte) und Residuen (rechts)}
\label{fig:scat1}
\end{figure}
...
\subsubsection{Einfache Lineare Regression - praktisches Beispiel 1}
...
\begin{enumerate}
\item Extrahieren und Aufbereiten der Daten
\item Visualisierung der Daten
\item Durchführen der Regressionsanalyse
\item Prüfung, ob Annahmen der Regressionsanalyse verletzt wurden
\end{enumerate}
...
\begin{figure}[H]
\centering
\includegraphics[width=1\textwidth]{images/prep1.png}\\[.25cm]
\caption{Scatterplots der Präpositionsdaten mit Regressionsgeraden}
\label{fig:prep1}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=1\textwidth]{images/prep2.png}\\[.25cm]
\caption{Diagnostische Grafiken der Präpositionsdaten}
\label{fig:prep2}
\end{figure}
...
\begin{enumerate}
\item Punkte mit extremen Werten, d.h. Werten $\ge$ 3 (um genau zu sein, Werten $\ge$ 3.29), sollten aus den Daten entfernt werden.
\item Falls mehr als 1\% der Datenpunkte Werte $\ge$ 2.5 haben (2.58 um genau zu sein), dann sind die Fehler unseres Models zu groß.
\item Falls mehr als 5\% der Datenpunkte Werte $\ge$ 2 haben (1.96 um genau zu sein), dann sind die Fehler unseres Models ebenfalls zu groß.
\end{enumerate}

Die rechte Grafik zeigt die \textit{studentized Residuen}, d.h. die angepassten vorhergesagten Werte jedes Datenpunkts werden durch den Standardfehler der Residuen dividiert. Auf diesem Weg ist es möglich die Student's t-Verteilung zu nutzen, um unser Model zu diagnostizieren.
...
\begin{figure}[H]
\centering
\includegraphics[width=1\textwidth]{images/prep3.png}\\[.25cm]
\caption{Weitere diagnostische Grafiken der Präpositionsdaten}
\label{fig:prep3}
\end{figure}
...
\subsubsection{Einfache Lineare Regression - praktisches Beispiel 2}
...
\begin{itemize}
\item[--]Gruppe A\\ 15, 12, 11, 18, 15, 15, 9, 19, 14, 13, 11, 12, 18, 15, 16, 14, 16, 17, 15, 17, 13, 14, 13, 15, 17, 19, 17, 18, 16, 14 \\
(mean: 14,93)
\end{itemize}
Die Schüler der zweiten Gruppe haben diese Punktzahlen erreicht.
\begin{itemize}
\item[--] Gruppe B\\ 11, 16, 14, 18, 6, 8, 9, 14, 12, 12, 10, 15, 12, 9, 13, 16, 17, 12, 8, 7, 15, 5, 14, 13, 13, 12, 11, 13, 11, 7 \\
(mean: 11,77)
\end{itemize}

Unsere Frage ist nun, ob Gruppe A wirklich besser ist oder ob das Ergebnis Zufall ist?

Gehen wir nun dazu über, die Regression in \verb!R! zu implementieren. Wie im vorherigen Beispiel leeren wir den gegenwärtigen Workspace, installieren und initialisieren/aktivieren notwendige Pakete und laden zusätzliche Funktionen.
...
\begin{figure}[H]
\centering
\includegraphics[width=.75\textwidth]{images/sprtest.png}\\[.25cm]
\caption{Darstellung der Sprachtestdaten}
\label{fig:sprtest}
\end{figure}
...
\subsection{(Multiple) Lineare Regression}\label{mlr}
...
\begin{itemize}
\item[--] Ist man nur an dem allgemeinen Modell-fit interessiert (ein Fall, der mir persönlich noch nie vorgekommen ist), sollte die Stichprobe mindestens 50 + k umfassen.
\item[--] Wenn man nur am Einfluss spezifischer Variablen interessiert ist, sollte die Stichprobe mindestens 104 + k umfassen.
\item[--] Wenn man an beidem interessiert ist, sollte man den je höheren Wert nehmen.
\end{itemize}
...
\subsubsection{Multiple Lineare Regression - praktisches Beispiel 1}
...
\begin{itemize}
\item[--] Datenpunkte mit standardisierten Residuenwerten $\ge$ 3.29 sollten entfernt werden \citep[269]{field2012discovering}
\item[--] Wenn mehr als 1\% der Datenpunkte standardisierte Residuenwerte $\ge$ 2.58, so ist der Fehleranteil des Modells inakzeptabel \citep[269]{field2012discovering}
\item[--] Wenn mehr als 5\% der Datenpunkte standardisierte Residuenwerte $\ge$ 1.96 haben, so ist der Fehleranteil des Modells inakzeptabel \citep[269]{field2012discovering}
\item[--] Zusätzlich sollten Datenpunkte mit Cooks D-Werten $\ge$ 1 entfernt werden\citep[269]{field2012discovering}
\item[--] Schließlich sollten Leveragewerte $3(k + 1)/n$ (k = Anzahl der Prädikatoren, N = Anzahl der Datenpunkte) nicht überschreiten \citep[270]{field2012discovering}
\item[--] Es sollte keine Autokorrelation innerhalb der unabhängigen Variablen vorliegen, d.h. eine Variable darf nicht mit sich selbst korrelieren (ist dies der Fall muss ein Repeated Measures Design oder ein hierarchisches Regressionsmodell genutzt werden)
\item[--] Unabhängige Variablen dürfen untereinander nicht stark korrelieren (Multikollinearität). Daher gilt, dass Datenpunkte mit Varianzinflationsfaktorenwerten (VIF) $\ge$ 10 entfernt werden sollten \citep{myers1990classical}. Es können aber bereits Werte von 2.5 problematisch sein \citep[215]{szmrecsanyi2006morphosyntactic}!
\item[--] Datenpunkte mit 1/VIF Werten $<$ .1 müssen entfernt werden (ab .2 gelten die Datenpunkte als problematisch) \citep{menard1995applied}
\item[--] Der Mittelwert der VIFs sollte $<$ 1 sein \citep{bowerman1990linear}
\end{itemize}
...
expR(m2.mlr)
\end{lstlisting}
Die Funktion \verb!smplesz! teilt mit, dass die Stichprobengröße nicht optimal ist und 9 Datenpunkte fehlen, um der Anforderung von \citet{green1991many} zu genügen. Die Wahrscheinlichkeit einen $\beta$-Fehler zu begehen ist hingegen sehr klein (0.0309). Als letzten Schritt tabellarisieren wir die Ergebnisse und fassen diese anschließend in Textform zusammen.
\begin{lstlisting}
...
Zusätzlich werden die Ergebnisse von multiplen linearen Regressionen schriftlich wie folgt zusammengefasst:\\[.2cm]
