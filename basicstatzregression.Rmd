...
m2.lm <- lm(Score ~ Group, data = slrdata2) # generate regression model
summary(m2.lm)                              # inspect results
```
...
# tabulate results
slrsummary2 <- slrsummary(m2.lm)
slrsummary2
```

```{r slr18, eval = T, echo=F, message=FALSE, warning=FALSE}
# tabulate results
slrsummary2 <- slrsummary(m2.lm)
library(knitr)
kable(slrsummary2, caption = "Results of a simple linear regression analysis.")
```
...
> Exercise Time!
>
> Load the mlrdata by executing the following command in R:

```{r slr19, eval = F, echo=T, message=FALSE, warning=FALSE}
slrdata3 <- read.delim("data/mlrdata.txt", header = TRUE)
```
