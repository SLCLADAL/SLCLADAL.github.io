...
# install packages
install.packages(c("tidyverse", "boot", "DescTools", "tidyverse", "psych", "Rmisc"))
```
...
```{r prep3, echo=F, message=FALSE, warning=FALSE}
...
```{r ds01, echo=F, message=FALSE, warning=FALSE}
...
```{r ds02, echo=F, message=FALSE, warning=FALSE}
# inspect data
df %>%
  head(20) %>%
  kable(caption = "Measures of central tendency and their use.") %>%
...
```{r ds03, echo=F, message=FALSE, warning=FALSE}
...
df %>%
  head(20) %>%
  kable(caption = "Sentences of the first paragraph of Herman Melville's *Moby Dick* and the number of words in each sentence.") %>%
  kableExtra::kable_styling(bootstrap_options = c("striped", "hover", "condensed", "responsive"))
```
...
df %>%
  head(20) %>%
  kable(caption = "Number of speakers across age groups in the private dialogue section of the Irish component of the  *International Corpus of English* (ICE).") %>%
  kableExtra::kable_styling(bootstrap_options = c("striped", "hover", "condensed", "responsive"))
```
...
df %>%
  head(20) %>%
  kable(caption = "Number of speakers across counties of current residency in the private dialogue section of the Irish component of the  *International Corpus of English* (ICE)") %>%
  kableExtra::kable_styling(bootstrap_options = c("striped", "hover", "condensed", "responsive"))
```
...
df %>%
  head(20) %>%
  kable(caption = "Performance of two stock packages.") %>%
  kableExtra::kable_styling(bootstrap_options = c("striped", "hover", "condensed", "responsive"))
```
...
particletable %>%
  head(20) %>%
  kable(caption = "Relative frequencies of discourse particles per speaker in two corpora.") %>%
  kableExtra::kable_styling(bootstrap_options = c("striped", "hover", "condensed", "responsive"))
```
...
```{r ds20, echo=F, message=FALSE, warning=FALSE}
particletable %>%
  ...
  geom_text(vjust=-2.5, position = position_dodge(0.9)) +
...
temprature %>%
  head(20) %>%
  kable(caption = "Average temperature in Hamburg and Moscow by month.") %>%
  kableExtra::kable_styling(bootstrap_options = c("striped", "hover", "condensed", "responsive"))
```
...
rts %>%
  head(20) %>%
  kable(caption = "Reaction times while sober and drunk.") %>%
  kableExtra::kable_styling(bootstrap_options = c("striped", "hover", "condensed", "responsive"))
```
...
Confidence intervals provide an estimation of in-between which values the reported value would lie in the population with a confidence of 95 percent. There are several ways to extract that information and to exemplify how to extract confidence intervals for values, we will continue to work with the reaction times data.
