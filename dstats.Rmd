...
sd(Moscow) # calculate standard deviation
```
...
wordstb %>%
  head(20) %>%
  kable(caption = "20 most frequent words in the sample corpus of ICE Ireland.") %>%
  kableExtra::kable_styling(bootstrap_options = c("striped", "hover", "condensed", "responsive"))
```
...
```{r ds33, echo = F, message=FALSE, warning=FALSE}
# define path to corpus
corpusfiles = list.files(path = "data/ICEIrelandSample", pattern = NULL, all.files = T, full.names = T, recursive = T, ignore.case = T, include.dirs = T)
# load and clean corpus
corpus <- lapply(corpusfiles, function(x) {
...
```{r ds45, message=FALSE, warning=FALSE}
Input =("
 Paw
 right
 left
