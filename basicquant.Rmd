...
Perhaps the most trivial and important issues related to the processing and analysis of data are the questions of why and how to use statistics. A simple question on this is that you use statistics to sample from a sample to a whole. It is purposely spoken of *an* instead of *the* whole, because there are many different universities and you should be very conscious about what total one wants to make statements. For example, if the sample includes only Germans between 20 and 30, the totality of the sample of conclusions allowed not the totality of all Germans, but only the totality of Germans between 20 and 30. Of course, this is very simplistic, as other factors in this example are left unconsidered. In this sense, statistics is a kind of help to simplification, which saves one from examining a totality, since a small group already gives information about what one would receive in a totality. The part of the statistic that deals with closing a sample to a whole is called inferential statistics, and we will look at the sections  on "Basic Interential Statistics".

Another reason is that you want to simplify complex issues. For example, you would like to compare two groups of students with each other and calculate the mean values of the scores achieved in a test that was taken by both groups. This part of the statistic is called descriptive statistics and we'll look at it in the section on descriptive statistics.

Before we focus on the different areas of statistics, however, we will first create basics and look more closely at basic concepts and their meaning. We do this in the following sections.
