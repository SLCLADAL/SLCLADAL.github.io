...
We add a dependent variable (AOI) which represents the dependent variable in the study. In our case, we determine that Condition has a relatively weak effect (the probability of gazing into the area of interest (AOI) is .3 in the test condition compared to .2 in the control condition).

```{r}
simdat <- simdat %>%
  dplyr::arrange(Condition) %>%
  dplyr::mutate(
  dep <- c(sample(c("AOI", "NotAOI"), 100, replace = T, prob = c(.2, .8)),
           sample(c("AOI", "NotAOI"), 100, replace = T, prob = c(.3, .7)))
  ) %>%
  dplyr::mutate_if(is.character, factor) %>%
  dplyr::rename(AOI = 4)
```
...
```{r pwr7, results='hide', fig.keep='all', message = FALSE, include = FALSE}
# load package
library(simr)
# set seed for replicability
set.seed(12345)
...
```{r pwr11, results='hide', fig.keep='all', message = FALSE, include = FALSE}
# set seed for replicability
set.seed(12345)
# perform power analysis for present model
...
# Power Analysis of Extending Data
...
```{r pwr13, results='hide', fig.keep='all', message = FALSE, include = FALSE}
...
```{r pwr15, results='hide', fig.keep='all', message = FALSE, include = FALSE}
pc2 <- powerCurve(m2, fixed("ConditionTest", "z"), along = "Item")
```
...
```{r pwr18, results='hide', fig.keep='all', message = FALSE, include = FALSE}
m3 <- extend(m1, along="Subject", n=30)
# perform power calculation
pc3 <- powerCurve(m3, fixed("ConditionTest", "z"), along="Subject")
```
...
```{r pwr21, , results='hide', fig.keep='all', message = FALSE, include = FALSE}
m4 <- extend(m1, within="Item+Subject", n=15)
# perform power calculation
pc4 <- powerCurve(m4, fixed("ConditionTest", "z"), within="Item+Subject", breaks=c(10, 12, 14))
```
...
The results show that we would have a sufficient data set if we had 14 observations per Subject in each Item because with 14 observations, the accuracy breaches the 80 percent level.
