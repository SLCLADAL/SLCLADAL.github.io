...
Based on the t-test, we assume that the data represents two samples from the same population (which is false). This means that the sample size is insufficient or *does not enough power* to show that they are actually from two different populations given the variability in the data nd the size of the effect (which is weak).
...
We can also add sujects and items *simultaneously* to address questions like *How many subjects would I need if I had 30 items?*. Hence, we increase both subjects and items from 10 to 30. This means that our *new* data/model has the following characteristics.
...
In this case, we return to each combination only occuring once.
