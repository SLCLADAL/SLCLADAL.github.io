...
---
\pagebreak\section{Describing data – Tables, variables, hypotheses, and data summaries}
\begin{abstract}
\noindent This chapter describes how variables are converted into measurable units (operationalization) and exemplifies how to set up tables by showing how to tabulate data and discusses what is understood by hypotheses, including different types of hypotheses (directed vs undirected hypotheses) and a classification/scales of variables (nominal, categorical, ordinal and numeric). In addition, this chapter introduces descriptive statistics including different measures of central tendency (mean, median, mode) and discusses when to use which of these measures as well as their advantages and disadvantages. Finally, this chapter introduces measures for variability such as the variance and the standard deviation. As such, this chapter sets the stage for the chapters to follow.
\end{abstract}

One of the most trivial but also most profound questions relating to data analysis is what statistical procedures should be used for. A simple answer to that question is that one uses statistics to draw inferences about the population from a sample of that population. In this sense statistics is a method that allows to simplify tasks by allowing to use a limited set of items instead of the entirety of all items. 
...
Bevor wir uns aber den verschiedenen Bereichen der Statistik widmen, werden wir zuerst Grundlagen schaffen und uns ganz grundlegende Begriffe und deren Bedeutung näher anschauen. Dies tun wir in den folgenden Sektionen.

\pagebreak
\subsection{Variablen}
Eine statistische Variable ist ein Merkmal, die eine bestimmte Ausprägung annehmen kann. Eine Variable kann, im Gegensatz zur Konstanten, mindestens zwei verschiedene Stadien oder Levels annehmen. Die Variablen in Ihrer Hypothese müssen operationalisiert werden, d.h. Sie müssen festlegen, was genau untersucht werden soll, was gezählt, was gemessen wird. In anderen Worten bezeichnet \textit{Operationalisierung} also die Messbarmachung einer Variablen. Eine Variable sollte möglichst so operationalisiert werden, dass ihre Messung möglichst gut
das zugrundeliegende Konzept wiedergibt. Die Entsprechung zwischen dem Konzept und der Variable, die das Konzept operationalisiert, wird als \textit{intrinsische Validität} bezeichnet. Ein Beispiel für intrinsische Validität wäre beispielsweise eine Operationalisierung von sprachlicher Komplexität, die diese gut messbar wiedergibt (beispielsweise Anzahl an Morphemen in einer Phrase, Anzahl der syntaktischen Knoten, o.ä.).

Das Gegenstück, \textit{externe Validität}, bezeichnet die Anwendbarkeit der Erkenntnisse, die man beispielsweise aus einer experimentellen Untersuchung gewonnen hat, auf die reale empirische Welt. So kann man in Experimenten nachweisen, welche Methoden dazu führen, dass man Nonsensesilben, d.h. Silben, die in der natürlichen L1 der Sprecher nicht vorkommen, besser erinnert. Externe Validität würde in diesem Zusammenhang bedeuten, dass man die Erkenntnisse auch auf andere Erinnerungsaufgaben in der wirklichen Welt, beispielsweise Vokabellernen, übertragen kann.
...
\item Stellen Sei sich folgendes Szenario vor: In einer großen Repräsentativen Studie wird herausgefunden, dass die Schuhgröße ein hervorragender Prädiktor (Vorhersagewert) für Intelligenz ist. Diskutieren Sie vor dem Hintergrund dieses Beispiels mit einem Partner ob intrinsische Validität notwendig ist?
\end{enumerate}
