...
```{r antc1, echo=FALSE, out.width= "70%", out.extra='style="float:left; padding:5px"', fig.cap="\\label{fig:Fig1} KWIC display of the search term language extracted from the BROWN corpus in the program AntConc."}
...
```{r coca1, echo=FALSE, out.width= "80%", out.extra='style="float:right; padding:5px"', fig.cap="\\label{fig:Fig2} Online KWIC display of the search term *language* extracted from the COCA corpus that is part of the BYU corpora."}
...
```{r skwic1, eval=T, echo=T}
# activate packages
lapply(c("quanteda", "dplyr", "stringr", "knitr", "kableExtra", "gutenbergr"), 
...
```{r skwic2, echo=T, eval = T, message=FALSE, warning=FALSE}
...
```{r skwic3, echo=T, eval = T, message=FALSE, warning=FALSE}
origin <- origin$text %>%
  paste0(collapse = " ") %>%
  str_squish()
str(origin)
...
```{r basekwic2, echo=T, eval = T, message=FALSE, warning=FALSE}
...
```{r basekwic8, echo=T, eval = T, message=FALSE, warning=FALSE}
...
```{r multikwic2, echo=T, eval = T, message=FALSE, warning=FALSE}
...
To include regular expressions in your KWIC searches, you include them in your search pattern and set the argument `valuetype` to `"regex"`. The search pattern `"\\bnatu.*|\\bselec.*"` retrieves elements that contain `natu` and `selec` followed by any characters and where the first character is at a word boundary. The `|` is an operator (like `+`, `-`, or `*`) that stands for *or*.
...
As R represents a fully-fledged programming environment, we can, of course, also write our own, customized concordance function. The code below shows how you could go about doing so but it only works if you enter more than one file. 
