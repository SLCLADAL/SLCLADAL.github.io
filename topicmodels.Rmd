...
This tutorial introduces topic modeling using R. Topic models aim to find topics (which are operationalized as bundles of correlating terms) in documents to see what the texts are about. Parts of this tutorial builds on and uses from  @wiedemann2017textmining. The entire code for the sections below can be downloaded [here](https://slcladal.github.io/rscripts/topicmodelsrscript.r).
...
After the preprocessing, we have two corpus objects: `processedCorpus`, on which we calculate an LDA topic model [@blei2003latent]. To this end, stopwords were removed, words were stemmed and converted to lowercase letters and special characters were removed. The second Corpus object `corpus` serves to be able to view the original texts and thus to facilitate a qualitative control of the topic model results.
...
```{r tm6, echo=T, eval = T, message=FALSE, warning=FALSE}
terms(topicModel, 10)
```

```{r tm7, echo=T, eval = T, message=FALSE, warning=FALSE}
...
```{r tm8, echo=T, eval = T, message=FALSE, warning=FALSE}
...
```{r tm9, echo=T, eval = T, message=FALSE, warning=FALSE}
exampleIds <- c(2, 100, 200)
lapply(corpus[exampleIds], as.character)
```

```{r tm10, echo=T, eval = T, message=FALSE, warning=FALSE}
exampleIds <- c(2, 100, 200)
...
```{r tm11, echo=T, eval = T, message=FALSE, warning=FALSE}
# see alpha from previous model
attr(topicModel, "alpha") 
```

```{r tm12, echo=T, eval = T, message=FALSE, warning=FALSE}
...
First, we try to get a more meaningful order of top terms per topic by re-ranking them with a specific score [@Chang]. The idea of re-ranking terms is similar to the idea of TF-IDF. The more a term appears in top levels w.r.t. its probability, the less meaningful it is to describe the topic. Hence, the scoring advanced favors terms to describe a topic.

```{r tm13, echo=T, eval = T, message=FALSE, warning=FALSE}
...
```{r tm16, echo=T, eval = T, message=FALSE, warning=FALSE}
...
```
```{r tm17, echo=T, eval = T, message=FALSE, warning=FALSE}
...
```{r tm18, echo=T, eval = T, message=FALSE, warning=FALSE}
...
Schweinberger, Martin. 2020. *Topic Modeling using R*. Brisbane: The University of Queensland. url: https://slcladal.github.io/topicmodels.html.
