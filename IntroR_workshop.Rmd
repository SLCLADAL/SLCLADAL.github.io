...
ICE_Ire_bio_texttype <- ICE_Ire_bio_selection2 %>%
  dplyr::mutate(Texttype = dplyr::case_when(stringr::str_detect(Filespeakerid ,"S1A") ~ "PrivateDialoge",
...
kwic_multiple <- as.data.frame(
  quanteda::kwic(Trump_split_clean, 
...
```{r}
ICE_Ire <- dplyr::left_join(ICE_Ire_split_tb, ICE_Ire_bio_edit, by = c("File", "Speaker"))
# inspect
head(ICE_Ire)
```
...
plotdata <- ICE_Ire %>%
  ...
  dplyr::filter(str_detect(File, "S1A"),
