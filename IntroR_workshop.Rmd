...
This document contains the materials for the *Introduction to R for (absolute) beginners* workshop offered by the [School of Languages of Cultures](https://languages-cultures.uq.edu.au/) at the [University of Queensland](https://www.uq.edu.au/). 

This workshop introduces R to (absolute) beginners and it specifically focuses on R for analyzing language data. 

## Goals of this workshop{-}

The goals of this workshop are:

- How to get stated with R
...
* Click [here](https://slcladal.github.io/IntroR_workshop.Rmd) to download the data for this session

* Click [here](https://slcladal.github.io/data/data.zip) to download the Rmd-file of this workshop
...
knitr::include_graphics("images/RStudio_newfolder.png")
```   
...
knitr::include_graphics("images/RStudio_empty.png")
``` 
...
knitr::include_graphics("images/RStudio_file.png")
``` 
...
knitr::include_graphics("images/RStudio_newfile.png")
``` 
...
knitr::include_graphics("images/RStudio_existingdirectory.png")
```  
...
```{r rstudio010, echo=F, fig.cap="", message=FALSE, warning=FALSE, out.width='50%'}
knitr::include_graphics("images/RStudio_file.png")
``` 
...
knitr::include_graphics("images/RStudio_newnotebook.png")
```  
...
```{r , echo=F, fig.cap="", message=FALSE, warning=FALSE, out.width='100%'}
knitr::include_graphics("images/RStudio_editMD.png")
```  
...
knitr::include_graphics("images/RStudio_run.png")
```  
...
In the example below, we create a new column called *Texttype*. This column contains the value *PrivateDialoge* if Filespeakerid contains the sequence *S1A*, the value *PublicDialogue* if Filespeakerid contains the sequence *S1B*,  the value *UnscriptedMonologue* if Filespeakerid contains the sequence *S2A*, the value *ScriptedMonologue* if Filespeakerid contains the sequence *S2B*, and Filespeakerid if Filespeakerid does neither contains *S1A*, *S1B*, *S2A*, nor *S2B*.

```{r}
ICE_Ire_bio_texttype <- ICE_Ire_bio_selection2 %>%
  mutate(Texttype = ifelse(str_detect(Filespeakerid ,"S1A") == T, "PrivateDialoge",
                    ifelse(str_detect(Filespeakerid ,"S1B") == T, "PublicDialogue",
                    ifelse(str_detect(Filespeakerid ,"S2A") == T, "UnscriptedMonologue",
                    ifelse(str_detect(Filespeakerid ,"S2B") == T, "ScriptedMonologue",
                           Filespeakerid)))))
# inspect data
...
Summarizing and grouping data (dplyr::group_by, dplyr::summarise)
...
We use this to show the power of regular expressions. Regular expressions are symbols or sequences of symbols that stand fro patterns. The symbol . stands for any character and * is a numerator regular expression that stands for 0 to an infinite number of instances. Thus, `.*` stands for 0 to an infinite number of any character. Also, if you put patterns in round brackets, R will remember the sequence within brackets and you can paste it back into a string from memory when you replace something. When referring to symbols that are used a regular expressions such as `\` or `\$`, you need to inform R that you actually mean the real symbol and not the regular expression and you do that by typing two `\\` before the sequence in question. Have a look at the example below and try to see what the regular expressions (`.*(S1A-[0-9]{3,3}).*`, `\n`, and `.*\\$([A-Z]{1,2}\\?{0,1})>.*`) stand for. 
...
# Getting started Data Visualizations in R{-}
