...
```{r detachpackages, echo = F, results='hide', message=FALSE, warning=FALSE}
#rm(list=ls(all=T))
...
```{r echo = T, results = 'asis'}
data02 <- read.delim("https://slcladal.github.io/data/data03.txt", sep = "\t", header = T)
data02
```
...
ggplot(linedata,                               # plot data from data set linedata 
  ...
  scale_x_discrete(breaks=c(4, 6, 8),        # customize x-axis tick positions
...
Ina next step, we load data the data.
...
# load files
corpus.files = list.files(path = corpus.path, pattern = NULL, all.files = T,
