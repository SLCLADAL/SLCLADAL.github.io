...
# Graphics with R
Before turning to the practical part, a few words on different frameworks for creating graphics in `R` are in order. There are three basic frameworks in which to create graphics in `R`. The basic framework, the lattice framework, and the ggplot framework. The basic framework is the oldest and is included in the `base`-package that is automatically activated when entering `R`. 

## The `basic` environment
The idea behind the `basic` environment is that the creation of graphics is seen in analogy to a painter who paints on an empty canvass. Each line or element is added to the graph consecutively which oftentimes leads to code that is very comprehensible but also very long.
...
ggplot(barplotdatagg1,                                    # plot barplotdatagg1
       aes(Decade, Frequency, fill = Variable)) +         # define x- and y-axis and grouping variable
  geom_bar(stat="identity", position=position_dodge()) +  # determine type of plot and grouping
  scale_fill_manual(values=c("goldenrod2", "gray70", "indianred4")) + # define colors
  geom_text(aes(label=Frequency), vjust=1.6, color="white",           # add text and define text color
...
```{r echo = T, results = 'asis'}
ggplot(iris,                                          # create a boxplot of Sepal.Length by Species
       aes(Species, Sepal.Length, color = Species)) + # define x-axis, y-axis and coloring factor
         geom_boxplot(outlier.colour="black",         # define outlier color
                      ...
                      fill=c("goldenrod2", "gray70", "indianred4"),  # define colors of boxes
...
ggplot(iris, aes(x=Species, y=Sepal.Length, , fill=Species)) + 
  ...
  labs(title="Sepal Length  by Species",x="Species", y = "Length (cm)") + 
