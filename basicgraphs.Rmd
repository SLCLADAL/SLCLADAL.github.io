...
As there exists a multitude of different ways to visualize data, this section can only highlight the most common types of visualization. Each type of visualization (or graph or plot) is briefly intrudued followed by code which produces the type of visualization in `R`. This section focuses on using `R` when dealing with plots because `R` is extremely flexible when it comes to cerating graphics. `R` is flexible in the sense that one can produce not only a huge variety of different types of visualization in `R` but also mody these visualizations to match one's individual needs. In addition, a major advantage of using `R` consists in the fact that the code can be store, distributed, and run very easily. This means that `R` represents a framework for creating graphs that enables sustainable, reproducible, and transparent procedures. 

# Basic Considerations about Visualizations
Before turning to the practical isseus relating to creating graphs, a few words on what one has to keep in mind when visualizing data are in order. On a very general level, graphs should be used to infrom the reader about properties and relationships between variables. This implies that graphs must be labelled properly to allow the reader to understand the visualization with ease. 
...
Althoug these are not scatterplots, plots with dot-symbols are very flexible and can be extended to show properties of the distribution of values. One way to create sucha plot is to plot means as dot-symbols and add error  bars to provide information wbout the underlying distribution. The plot below illustrates such a plot and additionally shows how plots can be further customized.
...
plot(MilesPerGallon_mean,             # plot MilesPerGallon_mean
     ...
     axes = F)                        # surpress drawing of axes 
lines(Speed_mean,                     # draw line for Speed_mean
...
axis(2,                               # y-axis 
     at = seq(10, 30, 5),             # tick marks at positions 10 to 30 with intervall 5 
...
ggplot(linedata,                               # plot data from data set linedata 
  ...
  geom_line(color = 'lightgray') +             # draw line and define color of line
...
Although the code for the `geom_smooth` line graph is much longer and requires addition specifications, it is a very nice way to protait the development of variables (here the midels per gallon and the quartermile speed) across the range of another variable (here the number of cylinders).

# Barplots
One very common data structure to visualize is tables conatining absolute frequencies or counts. The default way to visualize such frequency tables are barplots. This section deals with such bar plots and offers practical tips on how to modify and customize such plots.
...
barplot(counts,          # create barplot of counts
        ...
        xlab="Number of Gears",                # x-axis label
        col = c("grey30", "grey50", "grey70"), # color of bars
...
WE will briefly turn to pie charts because pie charts because they are frequently used although they have some severe shortcomings. Consider the follwoing example for we will cerate another data set to highlight the issues that arise when using pie charts.  

```{r echo = T, results='hide', message=FALSE, warning=FALSE}
Values <- c(10, 12, 7, 14, 9)                              # create a vactor of values
...
If the slices of the piechart were not lebelled, it would be difficult to see which slices are smaller or bigger comapred to other slices. This problem can easily be avoided when using bar charts.
...
Compared with the piechart, it is much easier to grasp the relative size and order of the percentage values of the countries. This goes to show that pie charts are unfit to show relationships between elements in a graph and, as a general rule of thumb, should be avoided.

Another feature of barplots is that the bars can be stacked and normalized so that changes in percentages become visible. One such example is shown below but before we can cerate sucha graph, we need to create an appropriate data set.
...
Stacked barplots should only be used when dealing with percentages because trends in the data will be more difficult to detect if the bars are of unequal heigth. Therefore, avoid stacking when dealing with uneven column sums. Another issue to consider is the number of variable levels: when dealing with amny variable levels, stacked barplots tend to become rather confusing, so either collapse infrequent variable levels or choose another visualization type (in cases when dealing with few frequent and many infrequent variable levels, line graphs tend to work better).

## Grouped Barplots
Another alternative to stacked barplots are grouped barplots. In addition to linegraphs, grouped barplots are particularly useful when dealing with frequency counts.  
...
barplot(                                 # create a barplot
  ...
  xlab="Decade",                         # add x-axis label
  ylab="Percent",                        # add y-axis label
  ylim=c(0,50),                          # definey-axis range
  col=c("darkblue","red", "gold"),       # define colors
...
After having dealt with barplots in `base`, we will now turn to barplots in `ggplot`.

## Barplots in `ggplot`
The logic of creating barplots in `ggplot` is again very similar to what we have encountered above. The code is also very similar as we only need to modify the data and change the specification of `geom_line` to `geom_bar`. 
...
barplotdatagg2 <- barplotdatagg1 %>%                    # create a data set called barplotdatagg2
  ...
  summarize(                                            # start summary 
    CumFreq = sum(Frequency)                            # create variable called CumFreq of cululative frequencies
  )                                                     # end data set creation
...
## Barplots for Questionaires
Barplots are particularly useful when visualizing data obtained through Likert items. As this is a very common issue that empirical researchers face. Accordingly, the following section exemplifies how to display such data using the Likert package. In a first step, we need to install the package (if it is not already installed).
...
After extracting a sample of the data,w e plot it to show how the Likert data can be displayed.

```{r echo = T, results='hide', message=FALSE, warning=FALSE}
plot(questionl28)
```

### Barplots with Negative Values

Another frequent task is to evaluate the divergen of values from a reference, for instance when dealing with language learners where native speakers serve as a reference or target. To illustrate how such data cane be visualized, we load the scales package as we want to cerate a barplot in which we show the divergence of learners from native speakers regarding certain features and how that divergence changes over time.  
...
We can now determine how the learners deviate from the native sepakers.
...
## Extensions of Barplots: Associationplot

A plot type that is related to barplots is the association plot. Association plots are similar to the barplot which shows deviations froma  reference as above but association plots show the difference between the observed and expected frequencies in bin. Therefore, they are often used when graphically representing tables with absolute frequencies. In order to plot an association plot, we need to load the `vcd` package. For the example here, we are also changing the labels in the table from "Male" and "Female" to "M" and "F" because otherwise, the labels would overlap.
...
Association plots are cretaed by using the `assoc` function which takes a table (or a similar format such as a matrix or a data frame) as their argument. In addition, we specify `shade` as T in order to ccolor code the bars in the association plot and to add a legend.
...
mosaic(HairEyeColor, # create a mosaic plot with the HairEyeColor data set
       ...
       legend=TRUE)  # add a legend to explain the color coding
```
 According to the mosaic plot above, people who have blond hair and brown eyes occur less frequently than expected if there were no correlation between eye and hair color (as indicated by the red rectangles in the lower left corner). The combinantion of blond hair and blue eys is, however, substantially more frequent than would be expected by chance. 

# Boxplots
So far we have plotted values but we have not plotted the underlying dsitributions. For instance, we have plotted mean values but not the variance within the distribution. One handy way to combine plotting general trends and their underlying distributions including their range are boxplots.

## Boxplots in `base`
Boxplots, or Box-and-Whisker Plots, are exploratory graphics frist cerated by John W. Tukey and they show the relationships between categorical and numeric variables. They are very useful because they not only provide measures of central tendency (the median which is the line in the middle of the box) but they also offer information about the distribution of the data. To elaborate, fifty percent of data points fall within the box  while seventyfive percent of data points fall within the wiskers (the lines which look like extended error bars): the box thus encompases the interquartile range between the first and third quartile. The wiskers show the minimum and maximum values in the data and only outliers (data points that lie 1.5 times the interquartile range or more above the third quartile or 1.5 times the interquartile range or more below the first quartile. If the whiskers differ in lenght, then this means that the data is asymmertically distributed.
...
Another interesting feature of boxplots is that they allow us to visuallly get an idea whether categories differ significantly. Because if add `notch = T` and the notches of the boxplots do not overlap, then this is a very strong indication that the categoreis actually differ significantly (see below). 
...
## Boxplots in `ggplot`
We will noe use the `ggplot` function to create boxplots.
...
### Extensions of Boxplots: Violine Plots

An alternative to boxplots which display the distribution within the adta even more accurately are violin plots. 
...
ggplot(iris,                                # use iris data
  ...
  geom_boxplot(width=0.1, fill="white")+    # create (additionaly) boxplots
  labs(title="Sepal Length  by Species",    # determine title
       x="Species", y = "Length (cm)") +    # determine axes labels
  scale_fill_manual(values=c("#999999", "#E69F00", "#56B4E9")) + # define colors
  theme_bw() +                              # use black ad white theme
  theme(legend.position="none")             # surpress legend
```

# Density Plots
Another way to visualize the distribution of the data with respect to numeric variables are density plots or Kernel Density Plots. Density plots smooth the data using so-called kernel smoothing to even out the distribution of frequencies along the lines of a numeric or intervall variable. The peaks of density plots help display where values are concentrated over the interval. To show the relationship between the variable and the density plot, we will first cerate a scatter plot and then create a density plot of the variable displayed on the x-axis of the scatter plot.
...
ggplot(iris,                                    # use iris data
  ...
  scale_color_manual(values = c('indianred4', 'darkgrey', 'gold')) + # define colors 
  theme(legend.position=c(0,1), legend.justification=c(0,1)) # add legend
```

We will now cerate a marginal density plot of Sepal.Length (x-axis).
...
ggplot(iris,                               # use iris data
  ...
  scale_fill_manual(values = c('indianred4', 'darkgrey', 'gold')) + # define colors
  theme(legend.position = "none")          # surpress legend
```
...
#```{r results='hide', message=FALSE, warning=FALSE}
corp <- Corpus(DirSource("D:\\Uni\\Korpora\\Original\\Wahlkampfprogramme Bundestagswahl 2013\\corpus"), readerControl = list(language = "german")) #specifies the exact folder where my text file(s) is for analysis with tm.
