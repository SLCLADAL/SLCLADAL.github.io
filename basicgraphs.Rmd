...
This tutorial introduces different types of data visualization. The entire R-markdown document for the tutorial can be downloaded [here](https://slcladal.github.io/rscripts/basicgraphs.Rmd).  
...
```{r prep3, echo=T, eval = T, message=F, warning=F}
# load data
...
The dat has the following format:
...
ggplot(plotdata, aes(x=Date, y= Prepositions, color = GenreRedux)) +
  ...
  scale_color_manual(         # define colours to be used
    values = c("indianred4", "gray30", "darkblue", "orange", "gray80"))
```
...
ggplot(plotdata, aes(x=Date, y= Prepositions,  color = GenreRedux, linetype = GenreRedux)) +
  ...
  # def. col.
  scale_colour_manual(values=c("goldenrod2", "gray30", "blue", "burlywood", "gray80"),
...
kable(head(likertdata), caption = "Table showing the first 6 lines of the likertdata") %>%
  kable_styling(bootstrap_options = "striped", full_width = T, position = "left")
```
...
kable(head(bardata), caption = "Table showing the first 6 lines of the bardata") %>%
  kable_styling(bootstrap_options = "striped", full_width = T, position = "left")
```
...
pie(bardata$Percent,
    col = c("red", "gray30", "blue", "gray80", "goldenrod"),
    labels = bardata$Percent)
legend("topright", names(table(bardata$DateRedux)), 
       fill = c("red", "gray30", "blue", "gray80", "goldenrod"))
```
...
ggplot(bardata,  aes("", Percent, fill = DateRedux)) + 
  ...
  scale_fill_manual(values = c("red", "gray30", "blue", "gray80", "goldenrod")) +
  theme_void()
```
...
kable(head(piedata), caption = "Table showing the first 6 lines of the piedata") %>%
  kable_styling(bootstrap_options = "striped", full_width = T, position = "left")
```
...
barplot(table(plotdata$DateRedux),        # plot Texts by DateRedux
     ...
     col = c("red", "gray30", "blue", "gray80", "goldenrod"), # add colors
     ylim = c(0, 250)                     # define y-axis limits
     )                                    # end drawing plot
grid()                                    # add grid
text(seq(0.7, 5.5, 1.2), table(plotdata$DateRedux)+10, table(plotdata$DateRedux))
box()                                     # add box
```
...
newplotdata <- plotdata %>%
    group_by(DateRedux, GenreRedux) %>%
  dplyr::summarise(Frequency = n())
kable(head(newplotdata), caption = "Table showing the first 6 lines of the newplotdata") %>%
  kable_styling(bootstrap_options = "striped", full_width = T, position = "left")
```
...
# bar plot
ggplot(newplotdata, aes(DateRedux, Frequency, fill = GenreRedux)) + 
...
ggplot(boxdata, aes(DateRedux, Prepositions, color = GenreRedux)) +                 
  geom_boxplot(fill=c("gold", "gray30", "indianred4", "blue", "gray80"), color="black") 
```
...
ggplot(boxdata, aes(DateRedux, Prepositions, color = GenreRedux)) +                 
  geom_boxplot(outlier.colour="red", outlier.shape=2, outlier.size=5, 
               notch=T, fill=c("gold", "gray30", "indianred4", "blue", "gray80"), 
               color="black") 
```
...
ggplot(boxdata, aes(DateRedux, Prepositions, fill = DateRedux)) +  
  ...
  scale_fill_manual(values = c("gold", "gray30", "indianred4", "blue", "gray80")) +
  theme_bw() +
  theme(legend.position = "none")         
```
