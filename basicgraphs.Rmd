...
This tutorial focuses on data visualization using R. The entire code for the tutorial can be downloaded [here](https://slcladal.github.io/rscripts/basicgraphsrscript.r). 
...
We now turn to data visualization using *ggplot*. As the *ggplot2* package is already loaded, we create a very basic scatterplot in *ggplot2* to show the advantages of cerating visualizations in this environment.
...
We are now in a position to start creating linegraphs with *ggplot*.
...
The labeling of pie charts is, however, somewhat tidious as the positioning is tricky. Below is an example for adding lables without specification.
...
To place the labels where they make sense, we will add another variable to the data called "Poisition".
...
We also modify the reduced plotdata as association plots work on matrices rather than dat frames or tibbles. In addition, we will drop more genres as to avoid overlap in the yaxis lables later on.
...
The bars above the line indicate that the observed frequency is higher than expected, bars under the line indicate frequencies that are lower than expected. However, the lack of coloring strongly suggests that there are noe significant differences between the observed and the expected frequencies which means that Genre is unlikly to have a significant effect of the frequency of prepositions.
...
According to the mosaic plot above, there are neither significant differences in the number of observations (the boxes are similiar in size and shape) and that genre does not have a significant effect on the frequency of prepositions as the boxes are grey and not colored. 
...
We again modify the data and create a matrix from the original plotdata. In addition, we scale the frequencies. This is not neccessary in the present case but when dealing with variables which differ in their mean and variance because they reflect different variables, scaling will normalize t´such variables and render their values comparable. 
...
The dendogram on the top shows that documents from 1600 and 1700 as well as documents from 1800 and 1900 are grouped together and thus are more similar with respect to their preposition frequencies. The dendogram on the left indicates that we have two categories of documents: the genres to towards the bottom tend to have fewer prepositions (indicated by the light colors) while the documents to the top tend to have more prepositions (thus the darker hues). Legal texts (genre = Law) have notably higher rates of prepositions as is deriveable from the dark red color of such texts.  
