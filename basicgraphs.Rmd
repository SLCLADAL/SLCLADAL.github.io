...
ggplot(plottable, 
  ...
  theme_bw() +                                                               # supress legend   
  guides(fill=FALSE) +                                                       # def. colours   
...
```{r wc2, echo = T, message=FALSE, warning=FALSE}
# specify path to corpus
corpus.path <- "https://slcladal.github.io/data/uspoliticalcorpus"
# load files
...
```{r wc3, echo = T, message=FALSE, warning=FALSE}
...
tdm <- as.matrix(tdm)                                     
# add lables
attr(tdm, "dimnames")$Docs <- c("Clinton", "Trump")       
```
...
```{r wc2, activatepackageswordcloud, echo = T, message=FALSE, warning=FALSE}
# load data
...
Clinton <- paste(as.vector(unlist(Clinton)), sep = " ", collapse = " ")
melville <- paste(as.vector(unlist(Trump)), sep = " ", collapse = " ")
# load libraries
library(tm)
library(dplyr)
library(xtable)
...
Now, we can cretae a comparison cloud.

```{r wc3, activatepackageswordcloud, echo = T, message=FALSE, warning=FALSE}
# create comparison cloud
