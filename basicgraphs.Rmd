...
There is a multitude of different ways to visualize data and thus we focus only on the most common in this chapter. Each type of plot is accompanied with the respective `R` code so that you can follow what we are doing on your own machine.  

### Simple scatter plots

Let's start of with a so-called "scatterplot", that is used when the relationship between two numeric variables should be shown. 
