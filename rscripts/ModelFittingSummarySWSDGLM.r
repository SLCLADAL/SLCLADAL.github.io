...
### --- Fixed-Effects Quasi-Poisson Regression Models
###########################################################################
##################################################################
...
### --- Fixed-Effects Quasi-Poisson Regression Models.
### --- NOTE
### --- This script only works for model comparisons of Fixed-Effects Quasi-Poisson Regression Models.
...
### --- Schweinberger, Martin. 2015. "Function for Step-Wise Step-Down Model
...
mdl.fttng.glm.swsd <- function(mdlcmp){
  require("stringr")
mdl.cmp.df1 <- sapply(mdlcmp, function(x) {
  ...
  add <- gsub("\n", " ", attr(x, "heading")[2], fixed = T)
  add <- gsub("Model [0-9]{1,5}: ", "~~~", add)
  add <- str_split(add, "~~~")
  add <- as.vector(unlist(add))
  add <- add[2:length(add)]
  add <- gsub(" {2,}", " ", add)
  ...
  p       <- round(x[[5]][[2]], 5)
  sig <-  p.nice(z = p)
  mdl.fttng <- cbind(add, df, dev, x2, x2df, p, sig)
}  )
mdl.cmp.df2 <- t(mdl.cmp.df1)
colnames(mdl.cmp.df2) <- c("Term added", "DF", "Residual Deviance", "X2", "X2DF", "p-value", "Significance")
...
# apply function
#test <- mdl.fttng.glm.swsd(mdlcmp)
# inspect results
#test
