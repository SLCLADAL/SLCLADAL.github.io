###########################################################################
### --- Function for Step-Wise Step-Down Model Fitting Process Summary of Model Comparisons of
### --- Mixed-Effects Binomial Logistic Regression Models
###########################################################################
##################################################################
### --- R script "Function for Step-Wise Step-Down Model Fitting Process Summary of Model Comparisons of
### --- Mixed-Effects Binomial Logistic Regression Models"
...
### --- saturated model including all effects and interactions
### --- The script only works for step-wise step-down model fitting!
...
### --- Schweinberger, Martin. 2014. "Function for Step-Wise Step-Down Model
...
### step-wise step-down model fitting
###############################################################
...
mdl.fttng.swsd.glmer <- function(mdlcmp){
mdl.cmp.df1 <- sapply(mdlcmp, function(x) {
    ...
    ifelse(length(attr(x, "heading")) == 5,
      ...
      ifelse(length(attr(x, "heading")) == 6,
        paste(gsub(" ", "", sub("m[0-9]{1,2}.glmer: ", "", attr(x ,"heading")[5])),
          gsub(" ", "", sub("m[0-9]{1,2}.glmer: ", "", attr(x ,"heading")[6]))),
      ifelse(length(attr(x, "heading")) == 7,
        paste(gsub(" ", "", sub("m[0-9]{1,2}.glmer: ", "", attr(x ,"heading")[5])),
          gsub(" ", "", sub("m[0-9]{1,2}.glmer: ", "", attr(x ,"heading")[6])),
          gsub(" ", "", sub("m[0-9]{1,2}.glmer: ", "", attr(x ,"heading")[7]))),
      ifelse(length(attr(x, "heading")) == 8,
        paste(gsub(" ", "", sub("m[0-9]{1,2}.glmer: ", "", attr(x ,"heading")[6])),
          gsub(" ", "", sub("m[0-9]{1,2}.glmer: ", "", attr(x ,"heading")[7])),
          gsub(" ", "", sub("m[0-9]{1,2}.glmer: ", "", attr(x ,"heading")[8]))),
          paste("error"))))))
  ...
  model <- ifelse(length(attr(x, "heading")) == 4, gsub(": .*", "", attr(x, "heading")[4]),
    ...
    ifelse(length(attr(x, "heading")) == 6, gsub(": .*", "", attr(x, "heading")[6]),
    ifelse(length(attr(x, "heading")) == 7, gsub(": .*", "", attr(x, "heading")[7]),
    ifelse(length(attr(x, "heading")) == 8, gsub(": .*", "", attr(x, "heading")[8]),
    "NA")))))
...
# TEST FUNCTION
# EXAMPLE
#mdlcmp <- list(m8.m7, m7.m6, m6.m5, m5.m4)
# apply function
#test <- mdl.fttng.swsd.glmer(mdlcmp)
# inspect results
#test
