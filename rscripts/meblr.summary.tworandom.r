...
meblrm.summary <- function(glm0, glm1, glmer0, glmer1, dpvar) {
  ...
  ranef <- c(names(summary(glmer1)[[9]][1]), round(summary(glmer1)[[13]][[1]][[1]],2),
  round(as.data.frame(VarCorr(glmer1))[[5]][[1]], 2),
    "", "", round(LLR01, 2), df, round(p, 4), p.nice(p))
  ranef2 <- c(names(summary(glmer1)[[9]][2]), round(summary(glmer1)[[13]][[2]][[1]],2),
  round(as.data.frame(VarCorr(glmer1))[[5]][[2]], 2),
    "", "", round(LLR01, 2), df, round(p, 4), p.nice(p))
...
# continue with setting up table
  ...
  ranef.tb2 <- rbind(ranef2)
  ranef.df2 <- as.data.frame(ranef.tb2)
  colnames(ranef.df2) <- colnames(coef.df)
  ...
  blr.tb <- rbind(ranef.df, ranef.df2, coef.df, gblstz.df)
  colnames(blr.tb) <- headranef
  rownames(blr.tb) <- c("Random Effect 1", "Random Effect 2", "Fixed Effect(s)", rownames(coefs),
