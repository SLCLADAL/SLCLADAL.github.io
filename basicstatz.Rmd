...
```{r skew01, echo = F, message=F, warning=F}
skew <- data.frame(rbeta(1000000,5,2)) %>%
  mutate(pskew = rbeta(1000000,2,5),
         nrm = rbeta(1000000,5,5)) %>%
...
ggplot(skew, aes(x = nskew, alpha = .5)) +
  ...
  geom_density(aes(x = pskew), fill = "orange", alpha = .5) +
  geom_density(aes(x = nrm), fill = "lightgray", alpha = .5) +
  theme_bw() +
...
To check for skewness, we load the `e1071` package which contains the `skewness` function that we will use to test for skewness here.

```{r skew04, message=F, warning=F}
skewness(words_women)            
```

If the reported skewness values is 
...
# add column names
colnames(ex1dat) <- c("", "1SGPN", "PN without 1SG", "Total")
# inspect data
ex1dat %>%
  as.data.frame() %>%
  flextable() %>%
...
The output table contains the configurations (here called *lables*), then the observed frequencies (the *counts*) and the expected frequencies, the Q and the $\chi^{2}$ statistic as well as the p-value asssociated with the $\chi^{2}$ value. In addition, the table provides the z-transformed $\chi^{2}$-values and their p-value.
