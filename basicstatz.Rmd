...
Another application in which the $\chi$^2^ test is often applied incorrectly is when ordinary Pearsons's $\chi$^2^ tests are used to test portions of tables with more than two rows and more than two columns, i. z*k tables (eg: row, k: column). An example is discussed by @gries2014frequency who also wrote the `R` Script for the correct version of the $\chi$^2^ test.
...
If we used an ordinary Pearsons's  $\chi$^2^ test (the use of which would be inappropriate here), it would reveal that spoken conversations do not differ significantly from fiction in their use of EMOTION IS LIGHT and EMOTION IS A FORCE OF NATURE ($\chi$^2^=3.3016, df=1, p=.069, $\phi$=.2057).
...
In order to perform the correct analysis, we must either implement the equation proposed in @bortz1990verteilungsfreie 144-148 or read in the function writen by @gries2014frequency and apply it to the subtable.
...
We will now begin the regression analysis by generating a first regression model. Once the regression model is generated, we will inspect its results and check if certain mathematical assumptions have been violated or whther the data conatins outliers. diagnostic plots.
...
*  If more than 5% of the data points have valuesgreater than 1.96, then the error rate of our model is too high.
...
The diagnostic plots are very positive and we will go through why this is so for each panel. The graph in the upper left panel is useful for finding ouliers or for determining the correlation between residuals and predicted values: when a trend becomes visible in the line or points (e.g., a rising trend or a zigzag line), then this would indicate that the model would be problematic (in such cases, it can help to remove data points that are too influential (outliers)).
...
```{r echo=T, message=FALSE, warning=FALSE}
# tabulate results
slrtb <- slr.summary(prep.lm)
library(knitr)
kable(slrtb, caption = "Results of a simple linear egression analysis.")
```
...
In addition, the results of simple linear regressions should be summarized in writing. An example of how the results of a regression  analysis can be written up is providede below.

A simple linear regression has been fitted to the data. A visual assessment of the model diagostic graphics did not indicate any problematic data points (outliers) or disproportionately influential data points and pointed to a good model fit. The final linear regression model is based on 603 data points and correlates highly significantly with the data (R^2^: 0.0108, F-statistic (1, 601): 6,553, p-value: 0.0107\*) and confirms a significant and positive correlation between the year in which the text was written and the relative frequency of prepositions  (coefficient: .02, standardized $\beta$: 0.1039, SE: 0.01, t-value: 2.560, p-value:. 0107\*).
...
Our question is whether group A has performed significantly better than group B which would indicate that the teaching method to which group A was exposed works better than the teachign method to which group B was exposed.
...
kable(sprtestdata, caption = "Data set for testing differences between groups using a simple linear egression analysis.")
```
...
col = c("lightgreen", "lightblue"))  # define colors
text(1:2,                            # add text
...
```{r echo=T, message=FALSE, warning=FALSE}
sprtest.lm <- lm(score ~ group, data = sprtestdata) # generate regressionmodel
...
library(knitr)
kable(slrtb2, caption = "Results of a simple linear egression analysis.")
```
