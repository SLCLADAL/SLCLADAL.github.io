...
So far, we have used the base `plot` function to generate maps. However, it is also possible to use `ggplot2` to generate maps when using `geom_sf` as shown below.

```{r geo1, message=FALSE, warning=FALSE}
# plot map
ggplot(data = world) +
  geom_sf()
```
