...
```{r sw3, eval = T, echo=F, message=FALSE, warning=FALSE}
# start search
sw1 <- ConcR(corpus, search.pattern1, context, all.pre = F)
sw2 <- ConcR(corpus, search.pattern2, context, all.pre = F)
sw3 <- ConcR(corpus, search.pattern3, context, all.pre = F)
sw4 <- ConcR(corpus, search.pattern4, context, all.pre = F)
sw5 <- ConcR(corpus, search.pattern5, context, all.pre = F)
sw6 <- ConcR(corpus, search.pattern6, context, all.pre = F)
sw7 <- ConcR(corpus, search.pattern7, context, all.pre = F)
sw8 <- ConcR(corpus, search.pattern8, context, all.pre = F)
sw9 <- ConcR(corpus, search.pattern9, context, all.pre = F)
sw10 <- ConcR(corpus, search.pattern10, context, all.pre = F)
sw11 <- ConcR(corpus, search.pattern11, context, all.pre = F)
# combine search results
swire <- rbind(sw1, sw2, sw3, sw4, sw5, sw6, sw7, sw8, sw9, sw10, sw11)
