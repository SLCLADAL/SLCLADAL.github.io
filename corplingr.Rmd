...
# Corpus Linguistics with AntConc and Microsoft Excel

This tutorial will exemplify how to perform a corpus analysis using AntConc, TextPad, and Microsoft Excel.
...
Für die praktischen Aufgaben zur Einführung in AntConc benötigen Sie das \textit{Brown University Standard Corpus of Present-Day American English} (BROWN Korpus) und müssen dieses herunterladen. Das Korpus kann auf folgender Seite heruntergeladen werden:\\
\verb!http://www.martinschweinberger.de/docs/data/BROWN2.zip!
\\
Nachdem Sie das Korpus heruntergeladen haben, entzippen Sie den Ordner und speichern ihn in einem Ordner, den Sie für das heutige Projekt/den Workshop angelegt haben bzw. anlegen.

## Downloading corpus data: : INNSBRUC

Für die praktischen Aufgaben zur Einführung in AntConc benötigen Sie das \textit{Innsbruck Letter Corpus} (INNSBRUC) und müssen dieses herunterladen. Das Korpus kann auf folgender Seite heruntergeladen werden:\\
\verb!http://www.martinschweinberger.de/docs/data/INNSBRUC.zip!
\\
Nachdem Sie das Korpus heruntergeladen haben, entzippen Sie den Ordner und speichern ihn in einem Ordner, den Sie für das heutige Projekt/den Workshop angelegt haben bzw. anlegen.

## Downloading the ICE Ireland metadata

Um die erste Übungsaufgabe bearbeiten zu können, müssen sie die Sprecherdaten der irische Komponente des \textit{International Corpus of English} (ICE), d.h. Informationen zu den soziolinguistischen Merkmalen der Sprecher wie beispielsweise deren Alter, Geschlecht, etc. Die Sprecherdaten können über folgenden Link heruntergeladen werden:\\
\verb!http://www.martinschweinberger.de/docs/data/BiodataIceIreland.txt!
\\
...
Um die zweite Übungsaufgabe bearbeiten zu können, müssen sie die Metadaten des INNSBRUC Korpus herunterladen, da Sie Informationen zu den Merkmalen der Briefe wie beispielsweise deren Author, Erscheinungsjahr, etc., benötigen. Die Metadaten können über folgenden Link heruntergeladen werden:\\
\verb!http://www.martinschweinberger.de/docs/data/INNSBRUC_meta.txt!
\\
...
Bevor wir uns der Übungsaufgabe widmen, werden wir zunächst einige Übungen durchgehen, die Ihnen helfen sollen die Funktionen von \verb!AntConc! besser kennenzulernen. Starten Sie AntConc und laden Sie das BROWN Korpus ein.\\
TIPP: Sie können sich sehr gute Einführungsvideos zu AntConc anschauen auf\dots \\
\verb!https://www.youtube.com/playlist?list=PLiRIDpYmiC0Ta0-Hdvc1D7hG6dmiS_TZj! \\[.5cm]
Übungsaufgaben: Einfache Konkordanzen
\begin{enumerate}
\item Wie häufig taucht das Wort \textit{linguistics} im BROWN auf?\\
%5
\item Satzinitiales \textit{but} \\
Was müssen Sie eingeben, um alle Fälle des satzinitialen \textit{but} zu extrahieren? (\textquote{Words} und \textquote{Case} angeklickt)?\\
%Zwei Möglichkeiten:
%\begin{enumerate}
%\item (nur wenn die Option "Case" angeklickt ist)\\
%But
%\item (diese Options findet nur Fälle von satzinitialem "but", wenn davor ein Punkt steht - sie wird nicht diejenigen Fälle finden, bei Fragezeichen, Semikolon oder Komma davor steht.\\
%. But
%\end{enumerate}
\item \textit{seems like} in verschiedenen Registern\\
Suchen Sie nach \textit{seems like} (eine eher informelle Phrase) und \textit{seems that} (eine formalere Phrase als \textit{seems like}) im BROWN J und anschließend, nachdem Sie BROWN J aus den aktiven Dateien in AntConc gelöscht haben, im BROWN A. Basierend auf Ihren Ergebnissen,\dots
\begin{enumerate}
\item können Sie erraten, welcher Teil des BROWN \textquote{science writing} und welcher \textquote{press reportages} repräsentiert? \\(TIPP: \textquote{press reportages} ist weniger formal)?\\
%BROWN J appears to be more formal than BROWN A as there is only 1 instance of \textit{seems like} but 5 instances of \textit{seems that} while there is one single instance of "seems like" but no instances of \textit{seems that} in BROWN J. Given this results, it seems likely that BROWN A is press and BROWN J is academic writing.
\item Welche Konstruktion wird in wissenschaftlichen Texten häufiger von L1 (US-amerikanischen) Englischsprechern verwandt? Waren Sie sich dieses Unterschieds bewusst?\\
%The construction \textit{seems that} is used more often by natives speakers of (American) English. I did not know that “seems that” is more formal than \textit{seems like}.\\
...
%\end{tabular}
%\end{footnotesize}
%\caption{Verteilung von \textit{seems like} und \textit{seems that} in BROWN A und BROWN J}
%\label{tab:seems}
%\end{center}
%\end{minipage} }
%\end{table}
\end{enumerate}
\end{enumerate}
Wild Cards (Reguläre Ausdrücke)\\
\textit{Wild Cards}, oder \textit{reguläre Ausdrücke}, sind Symbole die für andere, häufig generellere, Zeichenabfolgen stehen. Beispielsweise steht * für keinen oder mehrere Buchstaben während ein ? für genau einen Buchstaben steht. Um zu sehen, welche Wild Cards in AntConc genutzt werden können, schauen Sie bei \verb!Wildcard Settings! unter \verb!Global Settings! im Menü oben in der Leiste der AntConc GUI. WARNUNG: Wenn Sie Wild Cards benutzen, dann achten sei darauf, dass das Kästchen \verb!Regex! über dem Suchfenster angeklickt ist.
\begin{enumerate}
\item Mit welcher Zeichenfolge können Sie all Formen des Verbs \textit{walk} (d.h. \textit{walk}, \textit{walks}, \textit{walking}, \textit{walked}) finden?\\
%Lösung 1\\
%walk* (diese Lösung wird auch Treffer finden, die keine Formen des Verbs \textit{walk} sind, z.B. \textit{walkers}. Diese ungewollten treffer müsste man in einem späteren Schritt in Mirosoft Excel aussortieren.\\
%Lösung 2 (bessere Lösung)\\
%Gehen Sie zu \textquote{Advanced} im \textquote{Concordance} Reiter neben dem Suchfenster und schreiben Sie alle Formen des Verbs \textit{walk} in das obere Panel nachdem Sie das Kästchen \textquote{Use search term(s) from list below} angeklickt haben.

\item Was müssen Sie das Suchfenster eingeben, um alle Formen des Verbs \textit{sing} (d.h. \textit{sing}, \textit{sang}, \textit{sung}, \textit{singing}) zu erhalten?\\
%Lösung 1:\\
%s?ng* (Dieses ist die einzige wirklich richtige Lösung, da nur hier reguläre Ausdrücke verwendet werden. Alle ungewollten Treffer müssen später in Microsoft Excel aussortiert werden.)\\
%Lösugn 2\\
%sing|sang|sung|singing|sings\\
%Lösung 3\\
%Gehen Sie zu \textquote{Advanced} im \textquote{Concordance} Reiter neben dem Suchfenster und schreiben Sie alle Formen des Verbs \textit{sing} in das obere Panel nachdem Sie das Kästchen \textquote{Use search term(s) from list below} angeklickt haben.
\end{enumerate}
Weitere Anwendungsmöglichkeiten von AntConc
\begin{enumerate}
\item Kollokationen\\
Suchen Sie nach \textit{results} und gehen Sie anschließend auf den Reiter \verb!Collocates!. Klicken Sie erneut auf \verb!Start! und ordnen Sie die Ergebnisse nach \verb!Freq!. Zusätzlich beschränken Sie den Kontext auf 1 Wort links und 2 Wörter rechts indem Sie bei \verb!Window Span! 1L und 2R angeben. Welches ist die häufigste Kollokation von \textit{results}?\\
%the\\
Zusatz: Welches ist das häufigste nicht-funktionale Kollokat?\\
%obtained\\
\item Types und Token zählen\\
Wie viele Types und Token enthält das geladene Korpus?\\
Um diese frage zu beantworten, gehen Sie bitte auf den Reiter \verb!Word List! und klicken Sie auf \verb!Start!.
% 41518 Types und 1143801 Token
Zusatz: Warum liefert diese Methode nur ungenaue Werte?\\
\end{enumerate}

## Excercise 1: Swear words in Irish English

Die erste Übungsaufgabe, die wir bearbeiten, um die verschiedenen Schritte der Korpus- und Datenanalyse durchzuspielen, wird sein, dass wir der Frage nachgehen, ob Männer oder Frauen im irischen Englisch häufiger Schimpfwörter benutzen. Die notwendigen Schritte, um diese Frage empirisch beantworten zu können, sind folgende:

\begin{enumerate}
\item Korpusdaten in AntConc laden.
\item Einstellungen in AntConc anpassen.
\item Suchen nach einer Liste von Schimpfwörtern in den Daten mit wenig Kontext.
\item Erneutes suchen Schimpfwörtern in den Daten mit maximalem Kontext.
\item Speichern der Daten im Projektordner.
\item Säubern der Daten in TextPad.
\item Daten in Microsoft Excel kopieren.
\item Sprecher der Schimpfwörter extrahieren.
\item Absolute Häufigkeiten der Schimpfwörter pro Sprecher bestimmen.
\item Relative Häufigkeiten der Schimpfwörter pro Sprecher bestimmen.
\item Tabellarisieren der Mittelwerte der relativen Häufigkeiten hinsichtlich des Alters und Geschlechts der Sprecher.
\item Graphische Darstellung der Mittelwerte der relativen Häufigkeiten hinsichtlich des Alters und Geschlechts der Sprecher.
\item Statistische Auswertung der Ergebnisse mittels der erstellten Häufigkeitstabellen.
\end{enumerate}

Beginnen wir nun mit der ersten Übungsaufgabe.

### Extract swear words

Im folgenden werden die Arbeitsschritte aufgelistet, die notwendig sind, um die Fragestellung zu bearbeiten.
\begin{enumerate}
\item Erstellen Sie eine Liste mit Schimpfwörtern und speichern Sie diese Liste als txt-Datei in Ihrem Projektordner. WICHTIG: Jedes Wort (bzw. jede Sequenz) muss in einer eignen Zeile stehen.
\item Bevor Sie die Suche starten, gehen Sie bitte im Hauptmenü unter \verb!Tool Preferences! auf \verb!Concordance! und klicken Sie \\
...
\item Nutzen Sie AntConc, um diese Schimpfwörter im ICE Ireland Korpus zu suchen, indem Sie unter \verb!Advanced! das Kästchen \\ \verb!Use search term(s) from list below! anklicken, dann zu Ihrer Liste mit Schimpfwörtern browsen, dann auf \verb!Apply! klicken und anschließend auf \verb!Start! unter dem Suchfenster. \\
TIPP: Nutzen Sie reguläre Ausdrücke, sodass die Liste alle relevanten Wörter enthält und gleichzeitig nicht zu lang ist.
\item Anschließend exportieren sie die Daten, d.h. speichern Sie die txt-Dateien in Ihrem Projektordner indem Sie unter !File! im Hauptmenü auf \verb!Save Output to Text File! klicken und zu Ihrem Ordner navigieren. Nennen Sie die entstehende txt-Datei \\ \verb!cursewords-ire-2015-10-12-short!.
\item Nun suchen Sie erneut nach den Schimpfwörtern in Ihrer Liste allerdings erhöhen Sie nun den Kontext und geben dort den Maximalwert (1,000) ein.
\item Anschließend exportieren sie die Daten, d.h. speichern Sie die txt-Dateien in Ihrem Projektordner. Nennen Sie die entstehende txt-Datei \verb!cursewords-ire-2015-10-12-long!.
\end{enumerate}

## Cleaning data in TextPad
...
\begin{enumerate}
\item Öffnen Sie die Datei \verb!cursewords-ire-2015-10-12-short.txt! in TextPad. Wie Sie sehen, ist die Formatierung teilweise zerstört. In den nächsten Schritten werden wir die Formatierung wieder herstellen und die Ergebnisse säubern.
\item Klicken Sie auf \verb!f8! - es sollte sich nun ein \textquote{Suchen und Ersetzen} Fenster öffnen. Klicken Sie das Kästchen \textquote{Regulärer Ausdruck} an und suchen Sie anschließend nach \verb!\n<! und ersetzen Sie diese Sequenz durch \verb!<!. Wiederholen Sie dies, bis Ihnen TextPad anzeigt, dass keine Vorkommnisse von \verb!\n<! gefunden wurden.
\item Deaktivieren Sie das Kästchen \textquote{Regulärer Ausdruck} an und suchen Sie anschließend nach \verb! ! (zwei Leerzeichen) und ersetzen Sie diese Sequenz durch \verb! ! (ein Leerzeichen). Wiederholen Sie dies, bis Ihnen TextPad anzeigt, dass keine Vorkommnisse von \verb! ! (zwei Leerzeichen) gefunden wurden.
\item Klicken Sie wieder das Kästchen \verb!Regulärer Ausdruck! an und suchen Sie anschließend nach \verb!\n ! (Leerzeichen nach dem n) und ersetzen Sie diese Sequenz durch \verb!\n! (kein Leerzeichen nach dem \verb!n!). Wiederholen Sie dies, bis Ihnen TextPad anzeigt, dass keine Vorkommnisse von \verb!\n ! gefunden wurden.
\item Suchen Sie anschließend nach \verb!\n\t! und ersetzen Sie diese Sequenz durch \verb! ! (Leerzeichen). Wiederholen Sie dies, bis Ihnen TextPad anzeigt, dass keine Vorkommnisse von \verb!\n\t! gefunden wurden.
\item Markieren sie nun die Daten in TextPad (\verb!Strg!+\verb!a!) und Kopieren Sie sie (\verb!Strg!+\verb!c!) und fügen Sie sie in ein Spreadsheet in Mircosoft Excel (\verb!Strg!+\verb!v!) in die Zelle A2 ein. Löschen Sie leere oder unnötige Spalten und geben Sie den übrigen Spalten die Überschriften \textit{id}, \textit{pre}, \textit{token} und \textit{post}.
\item Wiederholen Sie diese Schritte für die Datei \verb!cursewords-ire-2015-10-12-long.txt!, allerdings fügen Sie diese Daten in das Spreadsheet neben den bereits eingefügten Daten ein und zwar so, dass die erste Zeile des long-Datensatzes neben der ersten Zeile des short-Datensatzes liegt.
\end{enumerate}
Übungsaufgaben: Weitere Anwendungsmöglichkeiten von regulären Ausdrücken in TextPad
...
\begin{enumerate}
\item Löschen Sie alle Spalten das long-Datensatzes außer die Spalte mit dem vorhergehenden Kontext, der Sie die Überschrift \textit{spk} geben.
\item Markieren Sie die \textit{spk}-Spalte (oben auf den Buchstaben klicken) und klicken Sie unter \verb!Suchen und Auswählen! auf \verb!Ersetzen!. In dem Pop-up Fenster geben Sie unter \textquote{Suchen nach} bitte \verb!*$! und unter \verb!Ersetzen durch! nichts ein. Anschließend klicken Sie auf \verb!Alle ersetzen!.
\item Markieren Sie die \textit{spk}-Spalte (oben auf den Buchstaben klicken) erneut und klicken Sie unter \verb!Suchen und Auswählen! auf \verb!Ersetzen!. In dem Pop-up Fenster geben Sie unter \verb!Suchen nach! bitte \verb!>*! und unter \verb!Ersetzen durch! nichts ein. Anschließend klicken Sie auf \verb!Alle ersetzen!.
\item Sie werden sehen, dass wir nun für sehr viele Schimpfwörter den Sprecher extrahiert haben. Um die restlichen Sprecher zu identifizieren, markieren Sie das gesamte Tabellenblatt (über die 1 und neben das A klicken) und gehen dann unter \verb!Sortieren und Filtern! auf \\ \verb!Benutzerdefiniertes Sortieren!. Klicken Sie das Kästchen \\ \verb!Daten haben Überschriften! an und sortieren Sie die Daten anschließend aufsteigend nach \textit{spk}. Auf diese Weise erscheinen alle Fälle, die keinen eindeutigen Sprecher haben am oberen Ende Ihrer Tabelle.
\item Bevor Sie manuell nach den Sprechern suchen, sollten Sie die Daten danach kodieren, ob der jeweilige Treffer wirklich auch ein Schimpfwort ist. Dies tun Sie, indem Sie in eine leere Spalte gehen, diese mit \textit{hit} überschreiben und dann, wenn es sich bei dem Treffer um ein Schimpfwort handelt, eine 1 eintragen, sonst eine 0.
\item Unglücklicher Weise müssen Sie die fehlenden Sprecher, die ein Schimpfwort verwendet haben, manuell suchen, indem sie einige Wörter (es müssen ganze, vollständige Wörter sein) aus dem Kontext des Treffers, d.h. aus der Zelle neben dem relevanten Token in der Spalte \textit{pre}, kopieren und nach dieser Sequenz im ICE Ireland mit AntConc suchen. Klicken Sie anschließend auf den Treffer im AntConc Konkordanzfenster (dies bringt Sie in Datei an den Ort an dem der Treffer ist) und suchen Sie nach dem Sprecher, den Sie dann manuell in die Excel Tabelle einfügen.
\end{enumerate}
...
\begin{enumerate}
\item Sie haben nun ein Tabellenblatt mit der Datei aus dem das Schimpfwort stammt, das Schimpfwort selber, den Kontext, in dem es verwendet wird, und den Sprecher, der es geäußert hat. Im nächsten Schritt werden wir bestimmen, ob bzw. wie häufig ein Sprecher Schimpfwörter verwendet hat.
\item Kopieren Sie die Sprecherdaten in ein neues Tabellenblatt und kopieren Sie die Spalten \textit{file}, \textit{spk} und \textit{sum}, die in der Spalte \textit{test} den Wert 1 haben, neben die Spalten mit der Sprecherinformation.
\item Nennen Sie die nächste freie Spalte \textit{afreq} und geben Sie folgenden Befehl in die erste Zelle dieser Spalte ein:\\
\verb!=ZÄHLENWENNS($J$2:$J$364;C2;$K$2:$K$364;D2)!
\item Kopieren Sie die Spalte \textquote{afreq} und fügen Sie sie wieder als Werte ein, d.h.: nachdem Sie die Spalten kopiert haben, machen Sie einen rechten Mausklick und geben Sie auf \textquote{Inhalte einfügen} und wählen Sie \textquote{Werte einfügen}.
\item Löschen Sie nun die Splaten \textquote{file}, \textit{spk} und \textit{hit} -- sie werden nicht mehr benötigt.
\item Kopieren Sie das entstandene Tabellenblatt.
\item Sortieren Sie die neue Tabelle aufsteigend nach \textit{word.count} und löschen Sie die Zeilen, die in der Spalte \textit{word.count} eine Wert kleiner als 50 haben. Anschließend sortieren Sie die Tabelle wieder benutzerdefiniert nach \textit{text.id} und als zweites nach \textit{spk.ref}.
\item Nennen Sie die nächste freie Spalte \textit{ptw} und geben Sie in die erste Zelle folgenden Befehl ein:\\
\verb!=J2/I2*1000!
%\item Anschließend sortieren Sie die neue Tabelle benutzerdefiniert (mit Überschriften) zuerst nach \textquote{file} und dann nach \textquote{spk}.
%\item Nenen Sie nun die nächsten beiden Spalten als \textquote{sum} und \textquote{test} und geben Sie folgenden Befehl in die erste Zelle der Spalte \textquote{sum}:\\
%\verb!=WENN(UND(A2=A1;B2=B1);C1+1;1)!
%\item Gehen Sie nun mit Ihrem Cursor auf die untere rechte Ecke der Zelle, in den Sie diesen Befehl eingegeben habe, bis ein schwarzes Kreuz erscheint und doppelklicken Sie mit der linken Maustaste.
%\item Gehen Sie nun in die erste Zelle der Spalte \textquote{test} und geben Sie folgenden Befehl ein:\\
%\verb!=WENN(UND(A2=A3;B2=B3);0;1)!
%\item Gehen Sie nun mit Ihrem Cursor auf die untere rechte Ecke der Zelle, in den Sie diesen Befehl eingegeben habe, bis ein schwarzes Kreuz erscheint und doppelklicken Sie mit der linken Maustaste.
%\item Kopieren Sie nun die Spalten \textquote{sum} und \textquote{test} und fügen Sie sie wieder als Werte ein, d.h.: nachdem Sie die Spalten kopiert haben, machen Sie einen rechten Mausklick und geben Sie auf \textquote{Inhalte einfügen} und wählen Sie \textquote{Werte einfügen}.
%\item Nun sortieren Sie die Tabelle benutzedefiniert absteigend nach \textquote{test}.
\end{enumerate}
...
\item Im letzten Schritt werden wir die gleichzeitig die Alters- und Geschlechterverteilung graphisch darstellen. Hierzu erstellen Sie bitte eine neue Pivottabelle, kopieren die Werte und fügen Sie als Werte wieder ein und wählen ein Säulendiagramm als Darstellungsart.
...
\begin{enumerate}
\item Erstellen Sie eine Übersichtstabelle mit den Anzahlen der Sprecher in der Textsorte \textit{Private Dialogue} hinsichtlich Alter und Geschlecht.
\item Erstellen Sie eine Übersichtstabelle mit den Wörterzahlen der Register und von den gegebenen Altersgruppen sowie getrennt nach Geschlecht.
\item Erstellen Sie eine Graphik, in der die Wörterzahlen von den Altersgruppen getrennt nach Männern und Frauen dargestellt werden.
\end{enumerate}
...
\begin{enumerate}
\item In der ersten Zelle der \verb!swear! Spalte geben Sie folgenden Befehl ein (unter der Bedingung, dass in Spalte \verb!D! die relativen Häufigkeiten der Schimpfwörter aufgelistet sind):\\
\noindent \verb!=WENN(D2=0; 1; 0)!\\
\item Gehen Sie nun mit Ihrem Cursor auf die untere rechte Ecke der Zelle, in den Sie diesen Befehl eingegeben habe, bis ein schwarzes Kreuz erscheint und doppelklicken Sie mit der linken Maustaste. Nachdem nun alle Zellen dieser Spalte angeben, ob der Sprecher Schimpfwörter genutzt hat oder nicht, fügen Sie den Inhalt der Spalte \verb!swear! wieder als Werte ein, d.h.: nachdem Sie die Spalte kopiert haben, machen Sie einen rechten Mausklick und geben Sie auf \textquote{Inhalte einfügen} und wählen Sie \verb!Werte einfügen!.
\item Markieren Sie die Tabelle, gehen Sie in der Registerkarte \textquote{Einfügen} auf \verb!Pivot Table! und klicken Sie in dem sich öffnenden Fenster auf \verb!OK!
\item Ziehen Sie in dem sich öffnenden Tabellenblatt \verb!sex! in den Bereich \verb!ZEILEN! und \verb!swear! in den Bereich \verb!WERTE!. Ziehen Sie nun \verb!sex! von oben in den Bereich \verb!WERTE! und klicken Sie anschließend auf \verb!OK!.
\item Im nächsten Schritt kopieren Sie die Tabelle und fügen Sie wieder als Werte ein, d.h.: nachdem Sie die Tabelle kopiert haben, gehen Sie auf ein freies Feld, machen einen rechten Mausklick, geben auf \verb!Inhalte einfügen! und wählen \verb!Werte einfügen!.
\item Gehen Sie nun neben die Zelle, die die Anzahl der weiblichen Sprecher enthält und ziehen Sie die Anzahl der Frauen, die Schimpfwörter genutzt haben, von der Gesamtanzahl der Frauen ab (262-91=171). Tun Sie dasselbe für die Männer. Kopieren Sie die Tabelle und fügen Sie sie als Wertetabelle wieder ein. Nun löschen Sie die Spalte, die die Gesamtzahl der Sprecher nach Geschlecht enthält - sie ist für das weitere Vorgehen irrelevant.
\item Berechnen Sie nun die Zeilen- und Spaltensummen, sowie die Gesamtsumme (TIPP: Nutzen Sie die Summenfunktion von Microsoft Excel).
\end{enumerate}
...
\hline\\
Beobachtete Werte & SwearwordUser & NoSwearwordUser & Zeilensumme \\[.3cm]
\hline
Female & 91 & 171 & 262\\
Male & 37 & 45 & 82 \\
Spaltensumme & 128 & 216 & 344\\
\hline\\
\end{tabular}
%\end{footnotesize}
\caption{Sprecher, die Schimpfwörter genutzt haben, nach Geschlecht}
\label{tab:obsswear}
\end{center}
\end{minipage} }
\end{table}
...
\begin{enumerate}
\item Such Sie vier freie Zellen unter der Wertetabelle. Berechnen Sie für jede Zelle den Wert, der zu erwarten wäre, wenn sich Frauen und Männer nicht unterscheiden, indem Sie die Zeilensumme mit der Spaltensumme multiplizieren und durch die Gesamtsumme dividieren.

\begin{equation}
\frac{Spaltensumme*Zeilensumme}{Gesamtsumme}
\label{eq:erw}
\end{equation}
\item Sie sollten nun folgende Tabelle errechnet haben:
...
\begin{tabular}{l c c c}
\hline\\
Erwartete Werte & SwearwordUser & NoSwearwordUser & Zeilensumme \\[.3cm]
\hline
Female & 97.49 & 164.51 & 262 \\
Male & 30.51 & 51.49 & 82\\
Spaltensumme & 128 & 216 & 344\\
\hline\\
\end{tabular}
%\end{footnotesize}
\caption{Erwartete Anzahlen von Sprechern, die Schimpfwörter genutzt haben, nach Geschlecht wenn kein Geschlechterunterschied vorhanden wäre}
\label{tab:obsswear}
\end{center}
\end{minipage} }
\end{table}

\item Im nächsten Schritt berechnen wir den $\chi^{2}$-Wert, indem wir für jede Zelle berechnen:\\
...
Erwartete Werte & SwearwordUser & NoSwearwordUser & Zeilensumme \\[.3cm]
\hline
Female & 0.432 & 0.256 & 0.688 \\
Male & 1.812 & 1.074 & 2.885 \\
Spaltensumme & 1.380 & 0.818 & 2.197 \\
\hline\\
\end{tabular}
%\end{footnotesize}
\caption{Tabelle mit X2-Werte}
\label{tab:obsswear}
\end{center}
\end{minipage} }
\end{table}

\item Die Summe aller $\chi^{2}$-Werte (2.197) ist nun der für uns relevante $\chi^{2}$-Wert. Um zu schauen, ob dieser Wert signifikant ist, müssen Sie zuerst die \textit{Freiheitsgrade} (degrees of freedom) berechnen und anschließend nachschauen, ob ihr Wert größer als ein kritischer $\chi^{2}$-Wert ist (dann sind Ihre Ergebnisse statistisch signifikant) oder ob ihr $\chi^{2}$-Wert kleiner als der kritische $\chi^{2}$-Wert ist (dann sind Ihre Ergebnisse nicht statistisch signifikant).
...
\label{eq:df}
\end{equation}
Nach unserer formal haben wir demnach einen Freiheitsgrad und wir können nun nachschauen, ob unser $\chi^{2}$-Wert (2.197) über oder unter dem kritischen Wert liegt. Sie finden Tabellen mit kritischen $\chi^{2}$-Werten in vielen Statistikeinführungen oder im WorldWideWeb, bspw. unter:\\

\noindent \verb!http://www.mesosworld.ch/lerninhalte/Biv_Chi/de/html/unit_SignKritWert.html!
...
# Excercise 2: Freqeuncy of *-ment* in the INNSBRUC Korpus
...
\subsection{Statistik mit dem Datenanalyse Add-In von Microsoft Excel}
...
Sie können mit Makros auch sehr viel komplexere Befehlsabfolgen aufzeichnen und ausführen lassen. Ich habe dazu ein Videotutorium auf meinem YouTube-Kanal:

\verb!https://www.youtube.com/watch?v=wzTT15ESGGg!
