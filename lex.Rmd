...
We can now use the `textstat_simil` function from the `quanteda` package to determine which degree adverbs are similar to each other and which degree adverbs are rather different. After applying the `textstat_simil` function, we then plot similarity scores which show which terms are most similar to each other.

```{r syn3, eval = F, message=FALSE, warning=FALSE}
similarity <- textstat_simil(dfmat_inaug_post1980, dfmat_inaug_post1980[c("2009-Obama", 
...
In addition, we can also use the `textstat_dist` function from the `quanteda` package to generate a distance matrix, which we can display as a dendrogram. 

```{r syn5, eval = F, message=FALSE, warning=FALSE}
