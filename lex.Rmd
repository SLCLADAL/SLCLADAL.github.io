...
# Finding synonyms: generating a custom thesaurus
...
syntb <- cosinesimilarity %>%
  as.data.frame() %>%
  mutate(word = colnames(cosinesimilarity)) %>%
  mutate_each(funs(replace(., . == 1, 0))) %>%
  mutate(synonym = colnames(.)[apply(.,1,which.max)]) %>%
  select(word, synonym)
syntb
```
...
# inspect results
datatable(head(translationtb, 100), rownames = FALSE, options = list(pageLength = 10, scrollX=T), filter = "none")
```
...
Schweinberger, Martin. 2020. *Lexicography with R*. Brisbane: The University of Queensland. url: https://slcladal.github.io/lex.html (Version 2020.12.03).
