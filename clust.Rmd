...
As the Hopkins statistic above shows, there is sufficient structure in the data and we can assume that there are actual clusters in the data. Next, we create a distance matrix based on euclidean distances.

```{r cl17, eval = T, echo =  T, message=FALSE, warning=FALSE}
clustd <- dist(clusts,                 # create distance matrix
               method = "euclidean")   # use euclidian (!) distance
```
...
We can also use other libraries to customize the dendrograms. 
