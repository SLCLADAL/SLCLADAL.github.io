...
This tutorial introduces classification using R. The entire code for the sections below can be downloaded [here](https://slcladal.github.io/rscripts/classificationscript.r). 
...
```{r ca1, eval = T, echo=T, message=FALSE, warning=FALSE}
# load libraries
library("FactoMineR")
library("factoextra")
...
```{r pca1, eval = T, echo=T, message=FALSE, warning=FALSE}
# inspect data
data(iris)
head(iris, 3)
```

```{r pca2, eval = T, echo=T, message=FALSE, warning=FALSE}
# log transform 
log.ir <- log(iris[, 1:4])
ir.species <- iris[, 5]
...
```{r pca3, eval = T, echo=T, message=FALSE, warning=FALSE}
# print method
print(ir.pca)
```


```{r pca4, eval = T, echo=T, message=FALSE, warning=FALSE}
# plot method
plot(ir.pca, type = "l")
```

```{r pca5, eval = T, echo=T, message=FALSE, warning=FALSE}
# summary method
summary(ir.pca)
```

```{r pca6, eval = T, echo=T, message=FALSE, warning=FALSE}
# predict PCs
predict(ir.pca, newdata=tail(log.ir, 2))
```


```{r pca7, eval = T, echo=T, message=FALSE, warning=FALSE}
# load library
#library(devtools)
...
```{r pca8, eval = T, echo=T, message=FALSE, warning=FALSE}
require(caret)
...
```{r pca9, eval = T, echo=T, message=FALSE, warning=FALSE}
...
```{r mds1, eval = T, echo=T, message=FALSE, warning=FALSE}
# Classical MDS
...
To show how vector space models work, we will follow the procedure described in @levshina2015linguistics to investigate similarities among amplifiers based on their co-occurrences (word embeddings) with adjectives. Adjective amplifiers are elements such as those in 1. to 5.
...
```{r vsm1, eval = T, echo=T, message=FALSE, warning=FALSE}
# load data
vsmdata <- read.delim("data/vsmdata.txt", sep = "\t", header = T)
# inspect data
str(vsmdata)
```

In a next step, we create a matrix from this data frame which maps how often a given amplifier co-occurred with a given adjective.

```{r vsm2, eval = T, echo=T, message=FALSE, warning=FALSE}
# load library
library(tm)
...
```{r vsm3, eval = T, echo=T, message=FALSE, warning=FALSE}
...
```{r vsm4, eval = T, echo=T, message=FALSE, warning=FALSE}
...
```{r vsm5, eval = T, echo=T, message=FALSE, warning=FALSE}
...
rect.hclust(cd, k = 6, col = "gray30", lty = "dotted", main = "", xaxis = "n")
```
