...
```{r line9, echo=T, eval = T, message=FALSE, warning=FALSE}
# activate packages
library(knitr)
library(lattice)             
library(ggplot2)               
...
likertdata1 <- read.delim("data/likertdata1.txt", 
                       sep = "\t", header = TRUE)
likertdata2 <- read.delim("data/likertdata2.txt", 
                       sep = "\t", header = TRUE)
# inspect data
...
```{r bar10a, eval = T, echo=T, message=FALSE, warning=FALSE}
# load data
...
```{r bar10b, eval = T, echo=T, message=FALSE, warning=FALSE}
# inspect data
...
This section introduces some statistical measures or tests that sueful when dealing with survey data. We will begin with measures of realiability (Cronbach's $\alpha$).
...
```{r cron01, echo = T, eval = T, message=FALSE, warning=FALSE}
# activate package
library(psych)
# load data
...
```{r cron02, echo = T, eval = T, message=FALSE, warning=FALSE}
...
```{r fa01, echo = T, eval = T, message=FALSE, warning=FALSE}
# remove respondent
...
```{r fa04, echo = T, eval = T, message=FALSE, warning=FALSE}
...
The plot shows that the questions form groups which indicates that the questions do a rather good job at reflecting the concepts that they aim to tap into.
...
```{r pca03, echo = T, eval = T, message=FALSE, warning=FALSE}
# entering raw data and extracting PCs  from the correlation matrix
PrincipalComponents <- princomp(surveydata, cor=TRUE)
summary(PrincipalComponents) # print variance accounted for
```

```{r pca05, echo = T, eval = T, message=FALSE, warning=FALSE}
loadings(PrincipalComponents) # pc loadings
```

```{r pca07, echo = T, eval = T, message=FALSE, warning=FALSE}
plot(PrincipalComponents,type="lines") # scree plot
```

```{r pca09, echo = T, eval = T, message=FALSE, warning=FALSE}
PrincipalComponents$scores # the principal components
```

```{r pca11, echo = T, eval = T, message=FALSE, warning=FALSE}
biplot(PrincipalComponents) 
```

```{r pca13, echo = T, eval = T, message=FALSE, warning=FALSE}
# Varimax Rotated Principal Components (retaining 3 components)
pcomp <- principal(surveydata, nfactors=3, rotate="varimax")
pcomp # print results 
```
