...
Der Median wird üblicher Weise bei ordinalen Variablen angegeben und er bezeichnet den mittleren Wert einer ab- oder aufsteigenden Folge numerischer Werte. Wenn die Reihe eine ungerade Anzahl von Elementen enthält, so werden die beiden mittleren Werte addiert und durch 2 dividiert (vgl. Formel \ref{eq:median}). In anderen Worten: Dies ist der mittlere Wert, d.h. 50\% der Werte befinden sich unterhalb und 50\% oberhalb des Medians. Bei einer geraden Anzahl an Daten ist der Median das arithmetische Mittel der beiden mittleren Werte.
\begin{equation}
\med x=
\begin{cases}
...
Der Modalwert wird häufig im Zusammenhang mit kategorischen Variablen verwendet und gibt an, welche Merkmalsausprägung am Häufigsten auftritt. Hier ein Beispiel und den Modalwert zu illustrieren: Nehmen wir an, Sie sind daran interessiert, wo die meisten Sprecher der privaten Dialoge in der irischen Komponente des \textit{International Corpus of English} (ICE) gegenwärtig residieren und sie erhalten folgende Verteilung.
...
dass Verteilungen Streuungsmaße bei nicht-normalverteilten Daten
parametrischen daten
