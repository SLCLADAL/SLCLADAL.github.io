...
The libraries quanteda and readtext are introduced by their developers in @welbers2017text  with brief examples – a useful text to read after our notebook.

With the packages loaded, the stage is set. Now we need to dress up the actors.

## From Data to Useable Objects

Before we can do anything further, we need to load the data into R. As mentioned earlier, we are working with the CORPS II corpus compiled by @guerini2010new. There are many ways to get the corpus into R, but in keeping with the binary theme of document classification, we walk you through two of them.

First for the easy version. By executing the following line of code, you tell R to download the CSV file containing the corpus from github and storing it in the session under the variable name `rt`:

```{r}
#rt = readtext(pointing to github repository)
```

In a slightly more complicated approach, you do the downloading manually from here, and then move it to a folder where you have a chance of finding it again. Then, replace the word `filepath` in the line of code below with the file path under which you saved the CSV and add the filename to the end of the file path. Then run the line.

```{r}
rt = readtext("C:/Users/maxlu/Documents/R/ATAP_ZurichD_DocClass/Data/SEL_perparty_v2.csv", text_field="text")
```

This is also a typical moment for failure. If you get an error message here, double-check the command. Make sure that you have the quotation marks around the file path, and that you copied the complete file path, including the full document name. Windows users should observe that R uses forward slashes for directories. Potential failure modes here are Umlauts and other symbols beyond the 26 letters and 10 numbers. Either muck around with the file path until you get it right, or revert to the easy version.
...
Let’s take a look at `rt`, the variable in which we stored the corpus using the `readtext` command. We can do this simply by entering the variable name:

```{r}
rt
```

The output tells us what we are dealing with: a readtext object consisting of 3247 documents and 3 document variables. We also see the first six rows of the object, which shows that we are dealing with a two dimensional matrix or a table, which in the R context is also called a data frame. What may be surprising is that, although the object is described as having three document variables, the data frame has five columns.

If we look at the CSV file of the corpus, we can see how that comes about. The CSV file contains four columns, labeled `party`, `fileid`, `name` and `text`, respectively. The object `rt` to which we assigned the data created an additional column, labeled `doc_id`.  In this variable, the row number of each row in the CSV file is stored. If we were to load in a second document and append it to the `rt` object, it would be clear which row of data comes from which corpus. So, while it’s great to have this `doc_id`, the output also makes it clear that it is not meaningful information relevant to the text we are interested in.

But, going back to the output, we still have on column more than we have document variables. If it is obvious to you why that is: mad props. If not, you can find out more about what the `readtext` command does by entering:

```{r}
?readtext 
```

You should see an information page loading in the bottom right window. The short description holds the answer:
...
```{r}
toks = tokens_remove(toks, pattern = stopwords(language = "en", source = "snowball"))
```
...
This time round, we create a new object with the name `dtm`, and the input to this operation is the latest version of the `toks` object. To see what the command `dfm` does in detail, you can open the help page using the command we used earlier:

```{r}
?dfm
```

This works for any command, and can come in extremely handy, both with the descriptions at the top of the page and with the list of flags, which gives a great overview over the specifications any given command takes. In this case, however, the description of the command does not tell us that much – unless you arrive with prior knowledge of what a document-feature matrix is: a sparse document-feature matrix, from a character, corpus, tokens, or even other dfm object?

In case you are not familiar with this particular type of matrix, you might get a better sense for what we are dealing with is by looking at the object directly:
...
Or, to put it into words: we create a new object called `train_dtm`, since we are going to train the model with this. The `train_dtm` object contains a sample of 2,800 speeches from the pre-processed and weighted `dtm` corpus.

```{r}
?dfm_sample
```

Looking at the description of the dfm_sample command, you can see that we have just taken a random sample with a size corresponding to roughly 85% of the corpus. There are no hard and fast rules as to which percentage of the data should be used for training and which for testing, but if you’re somewhere in the 90% ballpark for training data you should be doing fine.
...
If you look at `accuarcy`, you can see that it’s just a vector containing the integers 1 through 10:
...
As a heads up: if you should get an error message here, it is possible that your computer couldn’t handle the number of operations required to train twenty logistic regression models. If this is the case, we recommend rewriting the `accuracy` variable to only contain ten positions, i.e.:

```{r}
accuracy = 1:10
```

And then running the loop again.
...
```{r}
hist(accuracy, xlim=0:1, breaks=20)
```
