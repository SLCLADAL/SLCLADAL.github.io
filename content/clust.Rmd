...
The entire R Notebook for the tutorial can be downloaded [**here**](https://slcladal.github.io/content/clust.Rmd).  If you want to render the R Notebook on your machine, i.e. knitting the document to html or a pdf, you need to make sure that you have R and RStudio installed and you also need to download the [**bibliography file**](https://slcladal.github.io/content/bibliography.bib) and store it in the same folder where you store the Rmd file.</p>
...
A more elaborate and highly recommendable introduction to cluster analysis is @kassambara2017practical. Other very useful resources are, e.g.,  @king2015cluster; @kettenring2006practice; @romesburg2004cluster; and @blashfield1988methods.
...
```{r cl13, echo = F, message=F, warning=F}
...
```{r cl14b, echo = F, message=F, warning=F}
 clus %>%
  as.data.frame() %>%
...
```{r cl15b, echo = F, message=F, warning=F}
 clust %>%
  as.data.frame() %>%
...
* Values of 0 indicate that the data is uniform [@aggarwal2015data, pp. 158]. 
...
```{r cl17b, echo = F, message=F, warning=F}
 round(clustd, 2) %>%
  as.matrix() %>%
  as.data.frame() %>%
  tibble::rownames_to_column("Variety") %>%
...
```{r cl19, eval = F, echo =  T, message=FALSE, warning=FALSE}
clustd_maximum
```

```{r cl19b, eval = T, echo =  F, message=FALSE, warning=FALSE}
clustd_maximum  %>%
  as.matrix() %>%
  as.data.frame() %>%
  tibble::rownames_to_column("Variety") %>%
...
The clustering provides approximately unbiased p-values and bootstrap probability value [see @levshina2015linguistics 316].
...
```{r cl33b, eval = T, echo =  F, message=FALSE, warning=FALSE}
clus  %>%
  as.data.frame() %>%
  tibble::rownames_to_column("Variety") %>%
  flextable() %>%
...
```{r cl34b, eval = T, echo =  F, message=FALSE, warning=FALSE}
round(clustd, 2)  %>%
  as.matrix() %>%
  as.data.frame() %>%
  tibble::rownames_to_column("Variety") %>%
...
4. The child was *awfully*~amplifier~ *loud*~adjective~.
...
```{r vsm1b, eval = T, echo =  F, message=FALSE, warning=FALSE}
vsmdata  %>%
  as.data.frame() %>%
  head(10) %>%
  flextable() %>%
...
```{r vsm2b, eval = T, echo =  F, message=FALSE, warning=FALSE}
vsmdata_simp  %>%
  as.data.frame() %>%
  head(10) %>%
  flextable() %>%
