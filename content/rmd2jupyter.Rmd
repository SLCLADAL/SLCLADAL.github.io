...
```{r list, eval = F, message = F}
ladalrmds <- list.files(here::here("cbs"), pattern = ".Rmd")
ladalrmds
```
...
# convert 1 notebook
rmd2jupyter::rmd2jupyter(here::here("cbs", "textanalysis_cb.Rmd"))
```

This worked and a ipynb-file is now in our directory.

```{r convertall, eval = F, message = F}
