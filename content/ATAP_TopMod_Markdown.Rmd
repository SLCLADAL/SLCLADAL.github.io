...
The first step of the pre-processing requires us to transform the texts we stored under `dickens_corpus` and clean it by removing empty text elements using the `filter()` function from the `dplyr` package. Then, we group the data by title using `group_by()` (also a function from the `dplyr` package) and we then summarise the individual text elements (using the `summarise()` function from `dplyr`) by pasting them together using `paste0()` which results in one long string for each novel. :
