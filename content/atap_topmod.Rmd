...
This tutorial shows how to perform topic modelling using R. The entire R markdown document for the tutorial can be downloaded [here]("https://slcladal.github.io/content/atap_topmod.Rmd").
...
Before we begin installing and loading in the packages required for topic modelling, we need to install one piece of R-external software called Anaconda. Anaconda is a software that allows R to harness the additional computational capabilities of the Python language. Fortunately, Anaconda is extremely user-friendly, and has a graphic installer like any normal piece of accessible software - an .exe file for Windows and a .pkg file for MacOS. At the time of writing, the download link can be found here: https://www.anaconda.com/download. Pick the version you need, install Anaconda with the default settings, and let’s meet back here to continue in R.
...
Now that all of the packages are loaded, we need to set up the Python environment that we installed with Anaconda earlier, and make it ready to use for R. This only takes a single line of code, but can take some time if your computer already has some years on its back. Enter:
...
The first time you run this, you’ll see a prompt in the console. Choose the option which allows you to initiate the installation. Your console will keep you updated about all the installation steps that are running, which are quite a few over the next couple of minutes. Meet you back here with a cup of coffee (or rather the beverage of your choice, obviously). After installing this, you can mute the command in case you want to run this script again from the top.

Once the installation process is finished, we can initialize the environment which will allow R to use the Python functionalities in a minute. For this, we use:

```{r}
m_eng <- udpipe_load_model(file = here::here("udpipemodels", "english-ewt-ud-2.5-191206.udpipe"))

#spacy_intialize(model = "en_core_web_sm")
```
...
Before we go into the what and why of parsing, we suggest you initiate the process - it can take a while (depending on how good your device’s computing power is):

```{r}
dickens_parsed <- udpipe::udpipe_annotate(m_eng, x = dickens_corpus$text)

#dickens_parsed <- spacy_parse(dickens_corpus)
```

As you see, we take the `dickens_corpus`, transform it with the command `spacy_parse` and assign it to a new variable, `dickens_parsed`. What the command does is tokenize and tag the texts, returning a data-table of the result. The corpus is thus transformed from an object that has a sentence in each row, to one where each word has its own row. More importantly, though, each word is assigned a part of speech tag. Parsing text with spacy would also allow for lemmatization and more complex tags like noun phrases, but for our purposes, part of speech tags give enough information.
...
We are now dealing with a data frame that contains 2’004’243 observations (rows) of 7 variables (columns). Roughly two million words for eight novels sounds about right. Of course, if we want to be sure that we didn’t lose anything on the way thus far, we could go back to the Gutenberg information to see whether we have the correct amount of words. But we’ll carry on with the assumption that everything worked fine so far.
...
```{r}
no_prop_dickens <- dickens_parsed[dickens_parsed$pos !="PROPN",]
```
...
The first fifteen rows certainly indicate a success: gone is *Christmas* and *Carol*, gone is *Charles* and *Dickens*, not even *Prose* remains. Certain other features that are also not helpful for interpreting the content of the corpus currently remain, most prominently punctuation. Let’s take care of this.

# Tokenizing the Text

The first step is tokenizing the `no_prop_dickens`:
...
Our next target is punctuation. To remove it, the easiest way is to transform the `toks` object into a tokens object again (yes, it already is one), but this time we add a logical condition which states `remove_punct = TRUE`:
...
```{r}
toks <- tokens_remove(toks, pattern = stopwords("english"))
```
...
```{r}
stmOut_1 <- stm(documents=dtm_trimmed, K=10, max.em.its=200)
```
...
If your model is anything like ours, this is not yet what you’d call revelatory. There are some word pairs throughout the topics which go well together. For instance, we get *prisoner* and *prison* in one topic; *grave* and *earth* in another; *houses* and *windows*; *dog* and *guardian*; *magistrate* and *collector*.
...
```{r}
stmOut_2 <- stm(documents=dtm_trimmed, K=20, max.em.its=200)
plot(stmOut_2,n=10)
```
...
```{r}
stmOut_3 <- stm(documents=dtm_trimmed, K=20, max.em.its=200)
stmOut_3
stmOut_1
```
...
stmOut_4 <- stm(documents=dtm_trimmed, K=30, max.em.its=200)
```
...
stmOut_5 <- stm(documents=dtm_trimmed, K=25, max.em.its=200)
```
...
stmOut_6 <- stm(documents=dtm_trimmed_3, K=25, max.em.its=200)
plot(stmOut_6, n=10)
```
...
stmOut_7 <- stm(documents=dtm_trimmed_4, K=25, max.em.its=200)
plot(stmOut_7, n=10)
```
...
Schneider, Gerold  and Max Lauber. 2022. *Topic Modelling of Charles Dickens' novels*. Brisbane: The University of Queensland. url: https://ladal.edu.au/docs/atap_topmod.html (Version 2022.11.15).


```
@manual{schneider2022topmod,
  author = {Schneider, Gerold  and Lauber, Max},
  title = {Topic Modelling of Charles Dickens' novels},
  note = {https://ladal.edu.au/docs/atap_topmod.html},
  year = {2022},
