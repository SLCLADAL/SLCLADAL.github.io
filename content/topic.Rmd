...
The entire R Notebook for the tutorial can be downloaded [**here**](https://slcladal.github.io/content/topic.Rmd).  If you want to render the R Notebook on your machine, i.e. knitting the document to html or a pdf, you need to make sure that you have R and RStudio installed and you also need to download the [**bibliography file**](https://slcladal.github.io/content/bibliography.bib) and store it in the same folder where you store the Rmd file. <br><br>
...
For the next steps, we want to give the topics more descriptive names than just numbers. Therefore, we simply concatenate the five most likely terms of each topic to a string that represents a pseudo-name for each topic. We aim to achieve a  meaningful order of top terms for each topic by re-ranking them using a specific scoring method [@Chang2009].
...
# ... or have it selected by a term in the topic name (e.g. 'terror')
topicToFilter <- grep('terror', topicNames)[1] 
topicThreshold <- 0.2
