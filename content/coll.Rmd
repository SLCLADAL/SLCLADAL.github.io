---
title: "Analyzing Collocations, N-grams, and Keywords in R"
...
+ **keywords**: words that are over-represented in one context or collection (a text or a corpus) compared to another context or collection.
...
stats_tb %>%
  ...
  # round p-value
    ...
    # remove superfluous columns
    dplyr::select(-any_of(c("TermCoocFreq", "AllFreq", "NRows", "E12", "E21", 
                            "E22", "O12", "O21", "O22", "R1", "R2", "C1", "C2", "Exp"))) -> assoc_tb2
```
...
assoc_tb2 %>%
  ...
  ggplot(aes(x = reorder(token, DP, mean), y = DP)) +
  ...
  labs(x = "Token", y = "Association strength (Delta p(1|2))")
```
...
assoc_tb2 %>%
  ...
  # select the top 20 rows after sorting
  head(20) %>%
  # create a ggplot with 'token' on the x-axis (reordered by 'DP') and 'DP' on the y-axis
  ggplot(aes(x = reorder(token, DP, mean), y = DP, label = DP)) +
  # add a bar plot using the 'phi' values
  geom_bar(stat = "identity") +
  # add text labels above the bars with rounded 'phi' values
  geom_text(aes(y = DP - 0.005, label = round(DP, 3)), color = "white", size = 3) + 
...
To identify keywords, we can follow the procedure we have used to identify collocations using kwics - the idea is essentially identical: we compare the use of a word, for example, in corpus A to its use in corpus B.
...
Schweinberger, Martin. 2024. *Analyzing Collocations, N-grams, and Keywords in R*. Brisbane: The University of Queensland. url: https://ladal.edu.au/coll.html (Version 2024.03.26).
