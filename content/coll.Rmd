...
Now that we have all the relevant information, we can calculate the collocation statistics (the association strength).
...
# determine Term
  dplyr::filter(w1 == "selection",
    ...
    dplyr::arrange(-phi) %>%
    # remove superfluous columns
    dplyr::select(-any_of(c("TermCoocFreq", "AllFreq", "NRows"))) -> assoc_tb
# inspect results
head(assoc_tb, 10)
```
