...
This section focuses on how to describe and summarize data. The entire code for the sections below can be downloaded [here](https://slcladal.github.io/rscripts/descriptivestatzrscript.r). 
...
In linguistics three measures of centrality or measures of central tendency are of particular relevance: the *mean*, the *median* and the *mode*. In addition, there are two more measures of central tendency, the geometric and the harmonic mean which we will only briefly discuss as they are nto that relevant for language research. Which measure is appropriate depends on the type of variable scaling, the  distribution of the data, and what is the intended aim of the data summary.
...
In the following we will go over these types of measures of ecntral tendencies, exemplify their use, describe their strengths and weaknesses, and show how to calculate them in `R`.
...
Consider, we are interested in the mean length of sentences ina short text, then the first thing we could do would be to list the sentences and their length ina table.
...
The geometric mean represents a measure of central tendency that is used when dealing with dynamic processes where the later elements are dependent on the previous elements. The geometric mean is calculated according to the eqation below.

\begin{equation}
\bar{x}_{geometric} = \sqrt[n]{x_1 \times x_{i+1} \times \dots \times x_n}
\end{equation}

Imagine you have have the option to buy two different stock packages and you have to buy one of them. Which one would you buy?
...
The harmonic mean is a measure of central tendency that provides us with the average rate and is used when dealing with dynamic processes that involve velocities and distances. The harmonic mean is calculated according to the eqation below.
...
The reason why using the arithmetic eman is inappropriate in such cases is the following: The idea behind the arithmetic mean is that we calculate a single value that can replace all values in a given distribution and the sum of the te mean values is identical to the sum of the observed values. So the average is a single element that replaces each element. In our example, we have to drive at 40 kilometers per hour (instead of 30) to work and 40 kilometers per hour (instead of 60) to get back from work in the same amount of time. If we went with 45 kilometers pe hour, then the result would not be 3 hours but 2 hours and 40 minutes so that the result would not be the same.
...
The interquartile range (IQR) informs about how values are distributed and it denotes the range that encompasses 50 perecnt of data points. This means that the IQR spnas from the first quantile that encompasses 25 percent of the data to the third quartile that encompasses 75 percent of the data. 
...
The variance is calculated according to formula XXX (\ref{eq:varianz}). To calculate the variance, each value is subtracted from the mean and the result is squared. The squared values are then added and the resulting sum is divided by the number of values minus 1.
...
For our example, the equation (\ref{eq:sd1}) provides a standard deviation of 11.12 for city A and a standard deviation of 3.08 for city B.
...
Because this is a data-driven approach, the results will vary, depending on the characteristsics of the resampled data. To illustrate, comapre the values provided above to the values generated below.
...
The advantage of using bootstrapping method lies in teh fact that the data is (frequently) not distributed normally which is not an isseu for the bootstrapping and it will thus provide more reliable results as it does not rely on distributional assumptions about the data. 
