---
title: "Co-occurrence and Collocations"
...
```{r coll01, echo=T, eval = T, message=FALSE, warning=FALSE}
# load library
library(tm)
# load data
...
```{r coll03, echo=T, eval = T, message=FALSE, warning=FALSE}
...
```{r coll05, echo=T, eval = T, message=FALSE, warning=FALSE}
...
Next, we create a document-term-matrix. Only word forms which occur at least 10 times should be taken into account while an upper limit is not set.
...
```{r coll09, echo=T, eval = T, message=FALSE, warning=FALSE}
...
```{r coll21, echo=T, eval = T, message=FALSE, warning=FALSE}
# Convert to sparseMatrix matrix
require(Matrix)
DTM <- sparseMatrix(i = DTM$i, j = DTM$j, x = DTM$v,
                    dims = c(DTM$nrow, DTM$ncol),
                    dimnames = dimnames(DTM))
# extract cooccurrence counts
coocCounts <- t(DTM) %*% DTM
# inspect results
as.matrix(coocCounts[1:5, 1:5])
```

The matrix has `nTerms` rows and columns and is symmetric. Each cell contains the number of joint occurrences. In the diagonal, the frequencies of single occurrences of each term are encoded.

The part of the cooccurence matrix that is displayed shows that `r colnames(coocCounts)[4]` appears together `r coocCounts[4, 5]` times with `r colnames(coocCounts)[5]` in the `r nrow(binDTM)` sentences of the SUTO addresses. `r colnames(coocCounts)[4]` alone occurs `r coocCounts[4, 4]` times.

# Statistical significance

In order to identify which words occur together more frequently than would be expected by chance, we have to determine if their cooccurence frequency is statistical significant. Different significance-measures can be used. We need also various counts to calculate the significance of the joint occurrence of a term `i` (`coocTerm`) with any other term `j`:
...
These quantities can be calculated for any term (here `coocTerm`) as follows:

```{r coll21, echo=T, eval = T, message=FALSE, warning=FALSE}
library(dplyr)
library(tidyr)
coocdf <- as.data.frame(as.matrix(coocCounts))
coocStatz <- coocdf %>%
  dplyr::mutate(Term = rownames(coocdf)) %>%
  dplyr::arrange(Term) %>%
  tidyr::gather(CoocTerm, Frequency,
                colnames(coocdf)[1]:colnames(coocdf)[ncol(coocdf)]) %>%
  dplyr::filter(Term != CoocTerm) %>%
  dplyr::mutate(AllFreq = sum(Frequency)) %>%
  dplyr::group_by(Term) %>%
  dplyr::mutate(TermFreq = sum(Frequency)) %>%
  dplyr::group_by(Term, CoocTerm) %>%
  dplyr::mutate(TermCoocFreq = sum(Frequency)) %>%
  dplyr::ungroup() %>%
  dplyr::group_by(CoocTerm) %>%
  dplyr::mutate(CoocFreq = sum(Frequency))
coocStatz
```
...
We use the imported function `calculateCoocStatistics` to calculate the co-occurrences for the target term *"liberty"*.
