...
The entire R Notebook for the tutorial can be downloaded [**here**](https://slcladal.github.io/tagging.Rmd).  If you want to render the R Notebook on your machine, i.e. knitting the document to html or a pdf, you need to make sure that you have R and RStudio installed and you also need to download the [**bibliography file**](https://slcladal.github.io/bibliography.bib) and store it in the same folder where you store the Rmd file. <br><br>
...
<span>
<p style='margin-top:1em; text-align:center'>
The udipe R package also allows you to easily train your own models, based on data in CONLL-U format, so that you can use these for your own commercial or non-commercial purposes. This is described in the other vignette of this package which you can view by the command <br><br>
...
The dominant package for part-of-speech tagging with Python (the ugly, less versatile, and overall less good little brother of R) is *spaCy*. To be able to use the functionality of spaCy in R, we can use the `spacyr` package, which provides a convenient R wrapper around the Python spaCy package and allows to 

* pos-tag text data 
* parse texts into tokens or sentences  
* lemmatize tokens  
* dependency parsing   
* identify and extract named entities or noun phrases  

It also allows a user to request additional token-level attributes directly from spaCy.

What is neat about `spacyr` is that it takes care of the installation of spaCy and Python in a self-contained miniconda  environment while still using your R session (Python and the miniconda environment simply run in parallel in the background). Also, it is possible to install additional language models.

To be able to perform pos-tagging, dependency parsing and named entity recognition via spaCy, we need to install the `spacyr` and the `reticulate` packages.

```{r spacyr01, eval = F, message=F, warning=F}
install.packages("spacyr")
install.packages("reticulate")
install.packages("rpytools")
```

Then, we simply load `spacyr` and create a few sample sentences.

```{r spacyr02,  message=F, warning=F}
# load spacyr
library(spacyr)
# create sample data
...
We then initialize the `spacy` session from within R using the language model for English which is loaded automatically. Then, we can tag (or annotate to be precise) the example sentences and inspect the results.

```{r spacyr03,  message=F, warning=F}
# initialize spacy
...
We can also modify the output and show the dependencies and lemmas but, for example, not show pos-tags. 

```{r spacyr04,  message=F, warning=F}
# parse text with other output
modoutput <- spacy_parse(txt, dependency = TRUE, lemma = FALSE, pos = FALSE)
# inspect output
modoutput
```

Once we are done tagging with this model, we need to close the spacy session by using the `spacy-finalize` command.

```{r spacyr05,  message=F, warning=F}
# finalize spacy after being done
spacy_finalize()
```

We can then start another `spacy` session by initialize another `spacy` session from R in which we can pos-tag text with another language model.

```{r spacyr06,  message=F, warning=F}
# restart spacy
spacy_initialize(refresh_settings = TRUE)
```

Next, we can download another language model; in this case, we are downloading the German language model.

```{r spacyr07,  message=F, warning=F}
# download language  model for german
spacy_download_langmodel('de')
```

Then, we can initialize the model and pos-tag a German example text and inspect the results.

```{r spacyr08,  message=F, warning=F}
#spacy_load('de_core_news_sm')
spacy_initialize(model = "de_core_news_sm")
gertxt <- c(d1 = "Das hört sich gut an.",
         d2 = "Fritz, hol den Flammenwerfer!.")
# process documents and obtain a data.table
parsedtxt_ger <- spacy_parse(gertxt, tag = TRUE)
# inspect pos-tagged text
parsedtxt_ger
```


And, once again, if we are done tagging with this model, we need to close the `spacy` session by using the `spacy-finalize` command.

```{r spacyr09,  message=F, warning=F}
# finalize spacy
spacy_finalize()
```
