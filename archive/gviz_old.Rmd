---
title: "Geo-Spatial Data Visulization with R"
...
This tutorial focuses on geospatial data visualization using R. We already touched upon this in the [LADAL tutorial which introduces geospatial data visualization using R](https://slcladal.github.io/introgviz.html). In this tutorial, we will go further an add layers containing new information and adding interactivity to geospatial data visualization. 
...
The entire R Notebook for the tutorial can be downloaded [**here**](https://slcladal.github.io/gviz.Rmd).  If you want to render the R Notebook on your machine, i.e. knitting the document to html or a pdf, you need to make sure that you have R and RStudio installed and you also need to download the [**bibliography file**](https://slcladal.github.io/bibliography.bib) and store it in the same folder where you store the Rmd file. <br></p>
...
```{r prep2, eval = F, message=FALSE, warning=FALSE}
# set options
...
```{r pre2_new, eval = F, message=F, warning=F}
install.packages("sf")
...
```{r pre2_new, message=F, warning=F}
library(sf)
library(raster)
library(dplyr)
library(spData)
...
ggplot(arrivals, (aes(x = Longitude, y= Latitude))) +   
  ...
  stat_density2d(aes(fill = ..level..,  alpha = I(.2)),
                 size = 1, bins = 5, data = arrivals,
                 geom = "polygon") +
  geom_point(color="red", alpha = .2, size=arrivals$flights/100) +
  ...
  geom_text(aes(x=Longitude, y= Latitude, label=Name),
            color = "gray20", fontface = "italic", check_overlap = T, size = log(arrivals$flights),
            alpha = sqrt(arrivals$flights)/10)
```
...
```{r map_gg3, eval = F, echo = F, message=FALSE, warning=FALSE}
# load data
oz <- rnaturalearth::ne_countries(country = "australia", returnclass = "sf") 
# plot map
ggplot(oz) +
  geom_sf()
