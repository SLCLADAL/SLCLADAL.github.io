...
The [UQ Library](https://www.library.uq.edu.au/) offers a very handy and attractive summary of [ressources, concepts, and tools](https://guides.library.uq.edu.au/research-techniques/text-mining-analysis/introduction) that can be used by researchers interested in Text Analysis and Distant Reading. Also, the UQ library site offers short video introductions and addresses issues that are not discussed here such as [copyright issues](https://guides.library.uq.edu.au/research-techniques/text-mining-analysis/considerations), [data sources available at the UQ library](https://guides.library.uq.edu.au/research-techniques/text-mining-analysis/sources-of-text-data), as well as [social media](https://guides.library.uq.edu.au/research-techniques/text-mining-analysis/sources-of-text-data) and [web scaping](https://guides.library.uq.edu.au/research-techniques/text-mining-analysis/web-scraping).

In contrast to the UQ library site, the focus of this introducion lies on the practical how-to of text analysis. this means that the following concentrates on how to perform analyses rather than discussing their underlying concepts or evaluating their scientific merits. 


# Preparation

The following sections briefly introduce different types of analyses that are common among proceudes in Text Analysis. As all examples will be performed in "R", it is neccessary to install certain "libraries" so that the scripts shown below are executed without errors. Before turning to the code snippes below, please install the libraries needed for running the code below. If you have already installed the libraries mentioned below, then you can skip ahead ignore this section. To install the neccessary libraries, simply run the following code - it may take some time (between 1 and 5 minutes to install all of the libraries so you do not need to worry if it takes some time).
...
The entire code for the sections below can be downloaded [here](https://slcladal.github.io/rscripts/textanalysisscript.r).
...
Syntax trees are very handy because the allow us to check how reliable the parser performed. In the example above, the sentence was, in fact, parsed erroneously: the caluse conatining the string <just as the man has attended a funeral> is dependent on the relative clause <who gave him the book> although the first clause would correctly be dependent on the the clause <By chance Harry encountered the man>. However, other constituents are parsed correctly.
...
Based on this table, we can now vizualise the relative emotion scores for each book. 
...
We now create a document term matrix.
...
The topic modeling does, in fact, reflect the narrative topics of the three monographs with the first topic representing Orwell's "1984", the second topic representing Darwin's "On the origin of species", and the third topic representing Twain's "Adventures of Huckleberry Finn".

Topic Models can be refined and made more meaningful in various ways. For instance, the topic model shown here could be improved by splitting the monographs into chapters and running the analysis on the individual chapters rather than the entire monograph. Also, a very intersting application of Topic Modles would be to investiagte changes in topics over time which is dealt with in the tutorial that delves deeper into topic modelling.
...
The issue we want to investigate here relates to networks of personas in Shakespeare's "Romeo and Juliett".
