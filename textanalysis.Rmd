...
We begin by loading the `quanteda` package that we will use for generating the KWICs and the `tidyverse` package that we use to process the data. In addition, we load the data which represents the text of Charles Darwin's *On the Origin of Species*. 
...
# create kwic
kwic_o <- quanteda::kwic(x = darwin_chapters, 
                          pattern = "organism",
                          window = 5) %>%
...
textplot_network(tag_fcm, 
                 ...
                 edge_size = 5,
                 edge_color = "purple",
                 vertex_labelsize = log(rowSums(tag_fcm))*1.5)
```
