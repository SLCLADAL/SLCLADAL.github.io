...
word <- rep(rownames(collocates), each = ncol(collocates))
collocate <- rep(rownames(collocates), ncol(collocates))
collfreq <- as.vector(unlist(collocates))
# combine into data frame
dc <- data.frame(word, collocate, collfreq)
# load library 
library(igraph)
...
ggraph(bigram_graph, layout = "fr") +
  geom_edge_link(aes(edge_alpha = collfreq), show.legend = FALSE,
...
Now, this graph is not very appealing but it shows that there are relatively few words that collocate significantly in Darwin's "Origin". 
