...
# inspect data
nchar(darwin1); nchar(darwin2)
```

We now have the subsection of the data that we aim to investigate and can now perform the concordaning.
...
```{r conc5, echo=T, eval = T, message=FALSE, warning=FALSE}
...
```{r conc6, echo=T, eval = T, message=FALSE, warning=FALSE}
library(ggplot2)
...
```{r wf1, echo=F, eval = T, message=FALSE, warning=FALSE}
...
# load file content and start processing
corpus <- lapply(corpus.files, function(x) {
  x <- scan(x, what = "char", sep = "", quote = "", quiet = T, skipNul = T)
  x <- paste(x, sep = " ", collapse = " ")
  } )
# collapse data into one element
corpuswords <- paste(as.vector(unlist(corpus)), sep = " ", collapse = " ")
# remove non-words
corpuswords <- gsub("[^[:alpha:][:space:]]*", "", corpuswords)
# split data into individula words
corpuswords <- as.vector(unlist(strsplit(corpuswords, " ")))
# tabulate words in decending order of frequency
wordfreqs <- table(corpuswords)[order(table(corpuswords), decreasing = T)]
# convert table into a data frame
wordfreqs <- data.frame(names(wordfreqs), as.vector(wordfreqs))
# add column names
...
```{r wf2, echo=F, eval = T, message=FALSE, warning=FALSE}
# load library
library("wordcloud")
# create wordcloud
...
```{r wf3, echo=F, eval = T, message=FALSE, warning=FALSE}
# load data
