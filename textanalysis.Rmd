...
darwin <- readLines("https://slcladal.github.io/data/origindarwin.txt") %>%
  paste(sep = " ", collapse = " ") %>%
  tolower() %>%
  str_replace_all("(chapter [xvi]{1,4} {0,1})\\.{0,1}", "qwertz\\1") %>%
  strsplit("qwertz") %>%
  unlist()
# inspect data
nchar(darwin3)
...
# start concordancing
darwinnatsel <- ConcR(darwin3, "organism[s]{0,1}", 50)
# inspect data
darwinnatsel[1:5, 2:ncol(darwinnatsel)]
```

We now have the KWIC display but would like to clean the display a little bit. For instance, we would like to get rid of lines that do not contain any information (which happens if the search pattern does not occur in a subfile that we searched).
...
corpuswords <- Corpus(DirSource("https://slcladal.github.io/rscripts/data/testcorpus", encoding = "UTF-8"))  %>%
  paste(sep = " ", collapse = " ") %>%
  str_replace_all(" {2,}", " ") %>%
  tolower() %>% 
...
# create data frame
darwindf <- data.frame(darwin[1:length(darwin)-1], darwin[2:length(darwin)])
# add column names
...
prediction <- knn(train[2:ncol(train)], test[2:ncol(test)], cl = train[, 1], k = 3)
# inspect the result
prediction
