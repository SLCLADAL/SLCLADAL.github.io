...
The output shows that we are dealing with 20001 individual elements. Therefore, we combine these elements into one element and remove everything element that occurs after the header "CHAPTER II".

```{r conc2, echo=T, eval = T, message=FALSE, warning=FALSE}
...
We now have the subsections of the data that we aim to investigate and can now perform the concordancing. To cerate a KWIC display, we load the function "ConcR" from a script called "ConcR_2.3_loadedfiles.R". Then we define a pattern that we want to look for (the pattern can be a simple word or it contain regular expressions). Then, we define the amount of context that we want to have displayed (in our case 50 characters). Finally, we run the concordance function "ConcR" with the arguments "darwin3" the text elements taht we want to inspect, the search pattern, and the context.
...
We now have the KWIC display but would liek to clean the display a little bit. For instance, we would liek to get rid of lines that do not contain anay information (which happens if the serach pattern does not occur in a subfile that we searched).
...
To exemplify how frequency information can help us in an analysis, we will continue working with the KWIC display that we have cretaed above.
...
Now, we exctract the names of the chapters adn cretae a table with the chapter names and the relative frequency of matches per 1,000 words.
...
We can now vizualize the relative frequencies of our search word per chapter.
...
ggplot(tb, aes(x=chapters, y=relfreq, group =1)) + 
  ...
  scale_x_discrete(name ="Chapters")
...
corpuswords <- Corpus(DirSource("D:\\Uni\\UQ\\LADAL\\SLCLADAL.github.io\\data\\testcorpus", encoding = "UTF-8"))  %>%
  paste(sep = " ", collapse = " ") %>%
  str_replace_all(" {2,}", " ") %>%
  tolower() %>% 
...
Such word frequency lists can be visualized as word clouds. 
...
melville <- readLines("D:\\Uni\\UQ\\LADAL\\SLCLADAL.github.io\\data/melvillemobydick.txt")
# combine each text into one element
twain <- paste(as.vector(unlist(twain)), sep = " ", collapse = " ")
...
library(xtable)
# clean texts
docs <- Corpus(VectorSource(c(twain, melville))) %>%
...
```{r wf8, eval=T, echo=F, message=FALSE, warning=FALSE, paged.print=FALSE}
library(knitr)
...
This simple example shows that second person pronouns make up 2.6 percent of all words that are used in private dialogues while they only amount to 0.63 percent in scripted speeches. A handy way to present such differences vizually are association and mosaic plots.

```{r wf10, eval=T, echo=T, message=FALSE, warning=FALSE, paged.print=FALSE}
...
Bars above the dashed line indicate relative overse while bars below the line suggest relative underuse. Therefore, the associationplot indicates underuse of "you/your" and overuseuse of "other words" in monologoues while the opposite trends holds true for dialogues, i.e. overuse of "you/your" and undersuse of "Other words". 
...
In addition, words that collocate do not have to be immediately adjacent but can also encompass several slots. 
...
Parsing refers to another type of annotation in which either structural information (as in the case of XML documents) or syntactic relations are added to text. As syntactic parsing is commonly more relevenat in the language sciences, the follwoing will focus only on syntactic parsing. syntactic parsing builds on PoS-tagging and allows drawing syntactic trees or dependencies. Unfortunately, syntactic parsing is still has relatively high error rates when dealing with language that is not very formal. However, syntactic parsing is very reliable when dealing with written language.
...
Topic Modelling is a procedure that allows to extract clusters of key words. These key word clusters can represent topicsand the extraction and detection of such key word clusters builds on word frequencies and correlations between word frequencies. 
