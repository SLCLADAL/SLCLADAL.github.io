...
Just as a word of warning: when I set up my connection to Git and GitHUb things worked very differently, so things may be a bit different on your machine. In any case, I highly recommend [this YouTube tutorial](https://www.youtube.com/watch?v=kL6L2MNqPHg&t=716s) on how to get going with Git and GitHub when working woth RStudio.
...
To connect your Rproject and GitHub, open Rmd file in your Rproj and load the `usethis` package (you may need to install the `usethis` package if you have not already done so before.). To set everything up, I suggest that right after loading the `usethis` package, you look up the help page of the `use_github` function and scroll down to the **Git/GitHub Authentication** section. 


```{r usethis1, eval = F, message=F, warning = F}
library(usethis)
?use_github
```

```{r git_connect, echo=FALSE, out.width= "40%", out.extra='style="float:right; padding:10px"'}
knitr::include_graphics("https://slcladal.github.io/images/git_connect_rstudio2.png")
```

The **Git/GitHub Authentication** section says that you have to create a *personal access token* (PAT). So, we need to create a PAT by using the `create_github_token` function. 

```{r usethis2, eval = F, message=F, warning = F}
usethis::create_github_token()
```

If you are signed in to GitHub, the `create_github_token()` takes you to a pre-filled form to create a new PAT. You can now simply click on `Generate token` and copy(!) the PAT. In a next step, we need to store this PAT somewhere where RStudio can find it. To do this, use the `gitcreds::gitcreds_set()` function. If you do not already have a PAT, this will  it will prompt you to enter your PAT. Now, paste the PAT into the prompt!

```{r usethis3, eval = F, message=F, warning = F}
gitcreds::gitcreds_set()
```

In addition, you may need to define your user name and email as shown below.

```{r usethis4, eval = F, message=F, warning = F}
